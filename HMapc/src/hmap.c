...
const char *default_help_text =
    "By default, HMap prints out unique, successful IPv6/IPv4 addresses (e.g., "
...
static void *start_recv(void *args) {
    ...
    log_debug("hmap", "pinning receive thread to core %u", r->cpu);
...
static void *start_mon(void *args) {
    ...
    log_debug("hmap", "pinning monitor thread to core %u", mon_arg->cpu);
...
static void start_hmap(void) {
    log_debug("hmap", "hmap start.");
    ...
    log_debug("hmap", "source MAC address %02x:%02x:%02x:%02x:%02x:%02x",
              xconf.hw_mac[0], xconf.hw_mac[1], xconf.hw_mac[2],
              xconf.hw_mac[3], xconf.hw_mac[4], xconf.hw_mac[5]);
    ...
    if (!xconf.gw_mac_set) {
        ...
        log_debug("hmap", "found gateway IPv4 address %s on %s",
    ...
    log_debug("hmap", "gateway MAC address %02x:%02x:%02x:%02x:%02x:%02x",
...
#define PMAP_PF_ZC_CLUSTER_ID 9627
    ...
    if (xconf.pf.cluster == NULL) {
        log_fatal("hmap", "Could not create zc cluster: %s", strerror(errno));
    }
    ...
    for (uint32_t i = 0; i < user_buffers; ++i) {
        ...
        if (xconf.pf.buffers[i] == NULL) {
            log_fatal("hmap", "Could not get ZC packet handle");
        }
    }
    ...
    if (xconf.pf.send == NULL) {
        log_fatal("hmap", "Could not open device %s for TX. [%s]", xconf.iface,
                  strerror(errno));
    }
    ...
    if (xconf.pf.recv == NULL) {
        log_fatal("hmap", "Could not open device %s for RX. [%s]", xconf.iface,
                  strerror(errno));
    }
    ...
    for (uint32_t i = 0; i < xconf.senders; ++i) {
        ...
        if (xconf.pf.queues[i] == NULL) {
            log_fatal("hmap", "Could not create queue: %s", strerror(errno));
        }
    }

    xconf.pf.prefetches = pfring_zc_create_buffer_pool(xconf.pf.cluster, 8);
    if (xconf.pf.prefetches == NULL) {
        log_fatal("hmap", "Could not open prefetch pool: %s", strerror(errno));
    }
#endif

    // Initialization
    log_info("hmap", "probe network: ipv%d", xconf.ipv46_flag);
    log_info("hmap", "probe module: %s", xconf.probe_module->name);
    log_info("hmap", "output module: %s", xconf.output_module->name);
    log_info("hmap", "iid module: %s", xconf.iid_module->name);
    ...
    if (xconf.output_module && xconf.output_module->init) {
        if (xconf.output_module->init(&xconf, xconf.output_fields,
            ...
            log_fatal("hmap", "output module did not initialize successfully.");
        }
    }

    // Send global init
    iterator_t *it = send_init();
    if (!it) log_fatal("hmap", "unable to initialize sending component");
    ...
    if (!xconf.dryrun) { // TODO how many target will reply? <ip, port, index>
        ...
        log_debug("hmap", "bloomfilter for unique results <= 4e9");
        ...
        if (r != 0) {
            log_fatal("hmap", "unable to create recv thread");
        }
    ...
    for (uint8_t i = 0; i < xconf.senders; i++) {
        ...
        if (r != 0) {
            log_fatal("hmap", "unable to create send thread");
        }
    }
    log_debug("hmap", "%d sender threads spawned", xconf.senders);
    ...
    if (!xconf.dryrun) {
        ...
        if (r != 0) {
            log_fatal("hmap", "unable to create monitor thread");
        }
    }
    ...
    for (uint8_t i = 0; i < xconf.senders; i++) {
        ...
        if (r != 0) {
            log_fatal("hmap", "unable to join send thread");
        }
    }
    log_debug("hmap", "senders finished");

#ifdef PFRING
    pfring_zc_kill_worker(zw);
    pfring_zc_sync_queue(xconf.pf.send, tx_only);
    log_debug("hmap", "send queue flushed");
#endif
    ...
    if (!xconf.dryrun) {
        ...
        if (r != 0) {
            log_fatal("hmap", "unable to join recv thread");
        }

        bloom_filter_destroy(&xrecv.bf);
        log_debug("hmap", "receiver finished");

        if (!xconf.quiet || xconf.status_updates_file) {
            r = pthread_join(tmon, NULL);
            if (r != 0) {
                log_fatal("hmap", "unable to join monitor thread");
            }
        }
        log_debug("hmap", "monitor finished");
    }
...
static void deinit_state() {
    ...
    // deinit for target list
    if (xconf.targets.ips != NULL)
        freeIPVector(&xconf.targets);    
}
...
int main(int argc, char *argv[]) {
    ...
    if (args.download_given) {
        if (data_download(args.download_arg))
            log_info("hmap", "download and decompression Gasser et al's %s success", args.download_arg);
        else
            log_info("hmap", "download or decompression Gasser et al's %s failed", args.download_arg);
        exit(EXIT_SUCCESS);
    }
    ...
    if (xconf.log_file && xconf.log_directory) {
        log_init(stderr, xconf.log_level, xconf.syslog, "hmap");
        log_fatal("hmap", "log-file and log-directory cannot "
    ...
    if (xconf.log_file) {
        ...
        char       path[100];
        strftime(path, 100, "hmap-%Y-%m-%dT%H%M%S%z.log", local);
        char *fullpath =
    ...
    if (!log_location) {
        log_init(stderr, xconf.log_level, xconf.syslog, "hmap");
        log_fatal("hmap", "unable to open specified log file: %s",
                  strerror(errno));
    }
    log_init(log_location, xconf.log_level, xconf.syslog, "hmap");

    // hmap start
    log_debug("hmap", "hmap main thread started");
    if (config_loaded) {
        xconf.config_filename = args.config_arg;
        log_debug("hmap", "loaded configuration file %s", args.config_arg);
    }

    if (xconf.syslog) {
        log_debug("hmap", "syslog support enabled");
    } else {
        log_debug("hmap", "syslog support disabled");
    }
    ...
    if (!args.ipv6_given && !args.ipv4_given) {
        ...
        log_debug("hmap", "`-6' flag given, send ipv6 packet");
        xconf.ipv46_flag  = 6;
        xconf.ipv46_bytes = 16;
        xconf.ipv46_bits  = 128;
    } else if (args.ipv4_given) {
        log_debug("hmap", "`-4' flag given, send ipv4 packet");
        xconf.ipv46_flag  = 4;
        xconf.ipv46_bytes = 4;
        xconf.ipv46_bits  = 32;
    }
    ...
    if (args.max_len_given) {
        ...
        if (xconf.max_probe_len <= 0 ||
            ...
            log_fatal("hmap", "invalid max probe length: `-x|--max-len=%d'",
                      xconf.max_probe_len);
        }
        ...
        log_debug("hmap", "ipv%d max probing len=%d, +port=%d",
    ...
    if (!strcmp(args.output_module_arg, "default")) {
        log_debug("hmap", "no output module provided. will use `csv'.");
        ...
        if (!xconf.output_module) {
            log_fatal("hmap", "specified output module (%s) does not exist\n",
    ...
    if (!strcmp(args.probe_module_arg, "default")) {
        log_debug("hmap", "no probe module provided. will use `icmp_echo'.");
        xconf.probe_module =
    ...
    if (!strcmp(args.iid_module_arg, "default")) {
        log_debug("hmap", "no iid module provided. will use `low'.");
        ...
        if (!xconf.iid_module) {
            log_fatal("hmap", "specified iid module (%s) does not exist\n",
                      args.iid_module_arg);
        }
        xconf.iid_module_name = strdup(args.iid_module_arg);
        if (!strcmp(xconf.iid_module_name, "tga")) {
            SET_IF_GIVEN(xconf.list_of_ips_filename, list_of_ips_file);
            if (!xconf.list_of_ips_filename)
                log_fatal("hmap", "iid module tga,  please use -I to specify the path of the seed file.");
        }
    }
    if (args.iid_num_given && args.iid_num_arg < 0) {
        log_fatal("hmap", "`--iid-num=' should > 0\n", args.iid_module_arg);
    }
    ...
    if (xconf.fsconf.success_index < 0) {
        log_fatal("hmap", "probe module does not supply "
    ...
    if (xconf.fsconf.app_success_index < 0) {
        log_debug("hmap", "probe module does not supply "
    ...
    if (!strcmp(xconf.raw_output_fields, "*")) {
        ...
        for (int i = 0; i < xconf.output_fields_len; i++) {
            log_debug("hmap", "requested output field (%i): %s", i,
                      xconf.output_fields[i]);
        }
    ...
    if (!args.output_filter_arg || !strcmp(args.output_filter_arg, "default")) {
        ...
        log_debug("hmap", "empty output filter. will not exclude any values");
    } else {
        // Run it through yyparse to build the expression tree
        if (!parse_filter_string(args.output_filter_arg)) {
            log_fatal("hmap", "unable to parse filter expression");
        }
        // Check the fields used against the fieldset in use
        if (!validate_filter(xconf.filter.expression, &xconf.fsconf.defs)) {
            log_fatal("hmap", "invalid filter");
        }
        xconf.output_filter_str = args.output_filter_arg;
        log_debug("hmap", "will use output filter: %s", args.output_filter_arg);
    }

    // find if hmap wants any specific cidrs scanned instead
    ...
    if ((xconf.ipv46_flag == IPV4_FLAG) && xconf.blocklist_filename &&
        !strcmp(xconf.blocklist_filename, "/etc/hmap/blacklist4.conf")) {
        log_warn(
            "blocklist",
            "HMap is currently using the default blacklist located at "
            "/etc/hmap/blacklist4.conf. By default, this blacklist excludes "
            "locally scoped networks (e.g. 10.0.0.0/8, 127.0.0.1/8, and "
            "192.168.0.0/16). If you are trying to scan local networks, you "
            "can change the default blacklist by editing the default HMap "
            "configuration at /etc/hmap/hmap.conf.");
    }
    if ((xconf.ipv46_flag == IPV6_FLAG) && xconf.allowlist_filename &&
        !strcmp(xconf.allowlist_filename, "/etc/hmap/whitelist6.conf")) {
        log_warn("allowlist",
                 "HMap is currently using the default whitelist located at "
                 "/etc/hmap/whitelist6.conf. By default, this whitelist "
                 "includes 2001::/3. If you are trying to scan other networks, "
                 "you can change the default whitelist by editing the default "
                 "HMap configuration at /etc/hmap/hmap.conf.");
    }
    ...
    if (xconf.probe_module->port_args) {
        ...
        log_debug("hmap", "src_port range: [%d, %d]", xconf.source_port_first,
    ...
    if (args.batch_given) {
        ...
        if (xconf.batch < 0) {
            log_fatal("hmap", "invalid batch number: `--batch='");
        }
    }
    ...
    if (xconf.packet_streams < 0) {
        log_fatal("hmap", "invalid probe times: `--probes='");
    }

    // num-retries
    xconf.num_retries = args.retries_arg;
    if (xconf.num_retries < 0) {
        log_fatal("hmap", "invalid retry count: `--retries='");
    }

    // cooldown seconds
    SET_IF_GIVEN(xconf.cooldown_secs, cooldown_secs);
    if (xconf.cooldown_secs < 0) {
        log_fatal("hmap", "invalid cooldown secs: `-c|--cooldown-secs='");
    }
    ...
    // blocklist_init should default allow all addresss space
    // filename
    SET_IF_GIVEN(xconf.output_filename, output_file);
    SET_IF_GIVEN(xconf.list_of_ips_filename, list_of_ips_file);
    if (xconf.list_of_ips_filename) {
        log_debug("hmap", "init ip/seed file ing...");

        int64_t count = ip_target_file_init(xconf.list_of_ips_filename);
        if (count == -1) log_fatal("hmap", "init ip/seed file error");
        if (count == 0) {
            log_warn("hmap", "zero ip/seed file, no need to hmap");
            return EXIT_SUCCESS;
        }

        if (!strcmp(xconf.iid_module_name, "tga")) {
        // The HMap6 Target Generation Algorithm (INFOCOM'23)
            IPVector ipVector;
            initIPVector(&ipVector);
            initIPVector(&xconf.targets);

            FILE *fp;
            char  line[1000];

            fp = fopen(xconf.list_of_ips_filename, "r");
            if (fp == NULL)
                log_fatal("hmap", "unable to open seed file: %s: %s", xconf.list_of_ips_filename, strerror(errno));
            ...
            while (fgets(line, sizeof(line), fp) != NULL) {
                ...
                if (ipVector.size <= SEED_MAX_CAPACITY) {
                    addIP(&ipVector, seed2vec(ip));
                } else {
                    log_debug("hmap", "stop inserting seeds, reaching the maximum capacity of %d seeds", SEED_MAX_CAPACITY);
                    break;
                }
            }
            // Sort IP list
            qsort(ipVector.ips, ipVector.size, sizeof(char *), compareStrings);

            // Print sorted IP list
            // printf("Sorted IP list:\n");
            // for (size_t i = 0; i < ipVector.size; i++) {
            //     printf("%s\n", ipVector.ips[i]);
            // }
            
            log_debug("hmap", "ipVector created successfully, containing %d seed addresses", ipVector.size);

            hierarchical_cluster(&ipVector);

            HashSet* subspace_set = HashSet_init(INITIAL_HASH_SIZE);

            for (size_t i = 0; i < ipVector.size; i++) {               
                char *ptr = strchr(ipVector.ips[i], '*');
                if (ptr != NULL) 
                    HashSet_insert(subspace_set, ipVector.ips[i]);
            }

            log_debug("hmap", "generated %d scan subspaces", subspace_set->size);

            for (size_t i = 0; i < subspace_set->capacity; i++) {
                Node* current = subspace_set->table[i];
                while (current != NULL) {
                    // log_debug("hmap", "generate targets from subspace: %s", current->key);
                    if (xconf.targets.size <= TARGET_MAX_CAPACITY) {
                        target_generation(&xconf.targets, current->key, 0);
                    } else {
                        break;
                    }
                ...
                if (xconf.targets.size > TARGET_MAX_CAPACITY) {
                    log_debug("hmap", "stop generating targets, reaching the maximum capacity of %d targets, %.2f subspaces has been utilized.", TARGET_MAX_CAPACITY, (double) i / (double) subspace_set->capacity);
                    break;
                }  
            }

            count = xconf.targets.size;
            log_debug("hmap", "generated %d target addresses", count);
            
            // Free memory allocated by IPVector and Hashset
            freeIPVector(&ipVector);
            HashSet_free(subspace_set);
        }
        ...
        log_debug("hmap",
                  ...
                  xconf.list_of_ip_port_index_count);
        log_debug("hmap", "init ip target file completed");
        ...
        log_debug("hmap", "load ip target from file, max probe len reset to %d",
                  xconf.max_probe_len);
    }
    ...
    if (blocklist_init(xconf.allowlist_filename, xconf.blocklist_filename,
        ...
        log_fatal("hmap", "unable to initialize blacklist/whitelist");
    }
    ...
    if (mpz_zero(xconf.total_allowed_ip))
        log_fatal("hmap", "zero eligible addresses to scan");
    if (xconf.target_port_num)
    ...
    if ((args.shard_given || args.shards_given) && !args.seed_given)
        log_fatal("hmap", "need to specify seed if sharding a scan");
    ...
    if (xconf.min_hitrate > 0.0)
        log_debug("hmap", "scan will abort if hitrate falls below %f",
                  xconf.min_hitrate);
