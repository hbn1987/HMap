...
static inline void fs_add_word(fieldset_t *fs, const char *name, int type,
    ...
    if (fs->type == FS_REPEATED && fs->inner_type != type) {
        log_fatal("fieldset",
                  "object added to repeated field does not match type "
                  "of repeated field.");
    }
...
static char *sanitize_utf8(const char *buf) {
    ...
    for (uint32_t j = 0; j < i; j++) {
        ...
        if (safe_ptr == NULL) {
            log_warn("fieldset",
                     "UTF8 Sanitization issue. %u errors, fell through iter "
                     "%u. Orig: %s new: %s",
                     i, j, buf, safe_buf);
            i = j;
            break;
...
void fs_chkadd_string(fieldset_t *fs, const char *name, char *value,
                      int free_) {
    if (value) {
...
void fs_modify_string(fieldset_t *fs, const char *name, char *value,
                      int free_) {
    field_val_t val = {.ptr = value};
