...
iterator_t *send_init(void) {
    ...
    // process the source port range that HMap is allowed to use
    num_src_ports = xconf.source_port_last - xconf.source_port_first + 1;
    log_debug("send", "hmap will send from %i address%s on %u source ports",
...
int send_run(sock_t st, shard_t *sd) {
    ...
    if (!strcmp(xconf.iid_module_name, "tga")) {
        if (ip_target_list_get_ip(dst_ip, sd) == EXIT_FAILURE)
            memset(dst_ip, 0, xconf.ipv46_bytes);
        dst_port = ip_target_file_get_port(sd);
        index    = ip_target_file_get_index(sd);
    } else if(xconf.list_of_ips_filename) {
    ...
    while (1) {
        ...
        for (int u = 0; u < xconf.iid_num; u++) {
            if (!xconf.list_of_ips_filename) { // not get ip from file or target list
