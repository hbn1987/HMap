...
/* Module for querying specific UDP DNS resolvers with fake src IP addresses.
 *
 ...
 * www.qq.com.
 *
 * This module uses `-S, --source-ip=ip|range` as target resolver IPs, and
 * fills src IP addresses with generated addresses or IPs from files.
 *
 * So, there is no need to do any recv tasks.
 *
 ...
 *
 * Usage: xmap -p 53 --probe-module=dnsf --probe-args="raw:text:A,qq.com"
...
// xmap boilerplate
probe_module_t module_dnsf;
static int     dns_num_ports_f;

const char     default_domain_f[] = "www.qq.com";
const uint16_t default_qtype_f    = DNS_QTYPE_A;
const char    *dnsf_usage_error =
...
const unsigned char *charset_alpha_lower_f =
    (unsigned char *) "abcdefghijklmnopqrstuvwxyz";

static char    **dns_packets_f;
static uint16_t *dns_packet_lens_f; // Not including udp header
static uint16_t *qname_lens_f;      // domain_len list
static char    **qnames_f;          // domain list for query
static uint16_t *qtypes_f;          // query_type list
static char    **domains_f;         // domain strs
static int       num_questions_f   = 0;
static int       index_questions_f = 0;

/* Array of qtypes_f we support. Jumping through some hops (1 level of
 * indirection) so the per-packet processing time is fast. Keep this in sync
 * with: dns_qtype (.h) qtype_strid_to_qtype_f (below) qtype_qtype_to_strid_f
...
int8_t qtype_qtype_to_strid_f[256] = {BAD_QTYPE_VAL};

void setup_qtype_str_map_f() {
    qtype_qtype_to_strid_f[DNS_QTYPE_A]     = 0;
    qtype_qtype_to_strid_f[DNS_QTYPE_NS]    = 1;
    qtype_qtype_to_strid_f[DNS_QTYPE_CNAME] = 2;
    qtype_qtype_to_strid_f[DNS_QTYPE_SOA]   = 3;
    qtype_qtype_to_strid_f[DNS_QTYPE_PTR]   = 4;
    qtype_qtype_to_strid_f[DNS_QTYPE_MX]    = 5;
    qtype_qtype_to_strid_f[DNS_QTYPE_TXT]   = 6;
    qtype_qtype_to_strid_f[DNS_QTYPE_AAAA]  = 7;
    qtype_qtype_to_strid_f[DNS_QTYPE_RRSIG] = 8;
    qtype_qtype_to_strid_f[DNS_QTYPE_ALL]   = 9;
}

static uint16_t qtype_str_to_code_f(const char *str) {
    for (int i = 0; i < qtype_strs_len_f; i++) {
        if (strcmp(qtype_strs_f[i], str) == 0) return qtype_strid_to_qtype_f[i];
    }

    return 0;
}

static char    *label_f      = NULL;
static uint16_t label_len_f  = 0;
static uint16_t label_type_f = DNS_LTYPE_RAW;
static uint16_t recursive_f  = 1;

static uint16_t domain_to_qname_f(char **qname_handle, const char *domain) {
    if (domain[0] == '.') {
...
static int build_global_dns_packets_f(char **domains, int num_domains) {
    for (int i = 0; i < num_domains; i++) {
        qname_lens_f[i] = domain_to_qname_f(&qnames_f[i], domains[i]);
        if (domains[i] != (char *) default_domain_f) {
            free(domains[i]);
        }
        dns_packet_lens_f[i] =
            sizeof(dns_header) + qname_lens_f[i] + sizeof(dns_question_tail);
        if (dns_packet_lens_f[i] > DNS_SEND_LEN) {
            log_fatal("dnsf", "DNS packet bigger (%d) than our limit (%d)",
                      dns_packet_lens_f[i], DNS_SEND_LEN);
            return EXIT_FAILURE;
        }

        dns_packets_f[i]                = xmalloc(dns_packet_lens_f[i]);
        dns_header        *dns_header_p = (dns_header *) dns_packets_f[i];
        char              *qname_p      = dns_packets_f[i] + sizeof(dns_header);
        dns_question_tail *tail_p =
            (dns_question_tail *) (dns_packets_f[i] + sizeof(dns_header) +
                                   qname_lens_f[i]);

        // All other header fields should be 0. Except id, which we set
        // per thread. Please recurse as needed.
        dns_header_p->rd = recursive_f; // Is one bit. Don't need htons
        // We have 1 question
        dns_header_p->qdcount = htons(1);
        memcpy(qname_p, qnames_f[i], qname_lens_f[i]);
        // Set the qtype to what we passed from args
        tail_p->qtype = htons(qtypes_f[i]);
...
static uint16_t get_name_helper_f(const char *data, uint16_t data_len,
    ...
    if (recursion_level > MAX_LABEL_RECURSION) {
        log_trace("dnsf", "_get_name_helper OUT. ERR, MAX RECUSION");
        return 0;
    }
    ...
    while (data_len > 0) {
        ...
        // Is this a pointer?
        if (byte >= 0xc0) {
            log_trace("dnsf", "_get_name_helper, ptr encountered");
            // Do we have enough bytes to check ahead?
            if (data_len < 2) {
                log_trace("dnsf", "_get_name_helper OUT. ptr byte encountered. "
                                  "No offset. ERR.");
                return 0;
            }
            ...
            log_trace("dnsf", "_get_name_helper. ptr offset 0x%x", offset);
            ...
            if (recursion_level > 0 || bytes_consumed > 0) {
                ...
                if (name_len < 1) {
                    log_warn("dnsf", "Exceeded static name field allocation.");
                    return 0;
                }
            ...
            uint16_t rec_bytes_consumed = get_name_helper_f(
            ...
            if (rec_bytes_consumed == 0) {
                log_trace("dnsf", "_get_name_helper OUT. rec level %d failed",
                          recursion_level);
                return 0;
            } else {
                bytes_consumed += 2;
            ...
            log_trace("dnsf", "_get_name_helper, segment 0x%hx encountered",
                      byte);
            ...
            // null byte too)?
            if ((byte + 1) > data_len) {
                log_trace("dnsf", "_get_name_helper OUT. ERR. Not enough data "
                                  "for segment %hd");
                return 0;
            }
...
static char *get_name_f(const char *data, uint16_t data_len,
                        const char *payload, uint16_t payload_len,
                        uint16_t *bytes_consumed) {
    log_trace("dnsf", "call to get_name_f, data_len: %d", data_len);
    char *name      = xmalloc(MAX_NAME_LENGTH);
    *bytes_consumed = get_name_helper_f(data, data_len, payload, payload_len,
    ...
    log_trace("dnsf",
              "return success from get_name_f, bytes_consumed: %d, string: %s",
              *bytes_consumed, name);

    return name;
}

static bool process_response_question_f(char **data, uint16_t *data_len,
                                        const char *payload,
    ...
    uint16_t bytes_consumed = 0;
    char    *question_name =
        get_name_f(*data, *data_len, payload, payload_len, &bytes_consumed);
    // Error.
    ...
    if (qtype > MAX_QTYPE || qtype_qtype_to_strid_f[qtype] == BAD_QTYPE_VAL) {
        ...
        fs_add_string(qfs, "qtype_str",
                      (char *) qtype_strs_f[qtype_qtype_to_strid_f[qtype]], 0);
    }
...
static bool process_response_answer_f(char **data, uint16_t *data_len,
                                      const char *payload, uint16_t payload_len,
                                      fieldset_t *list) {
    log_trace("dnsf", "call to process_response_answer_f, data_len: %d",
              *data_len);
    // Payload is the start of the DNS packet, including header
    // data is handle to the start of this RR
    // data_len is a pointer to the how much total data we have to work
    ...
    if (type > MAX_QTYPE || qtype_qtype_to_strid_f[type] == BAD_QTYPE_VAL) {
        fs_add_string(afs, "type_str", (char *) BAD_QTYPE_STR, 0);
    } else {
        // I've written worse things than this 3rd arg. But I want to be
        // fast.
        fs_add_string(afs, "type_str",
                      (char *) qtype_strs_f[qtype_qtype_to_strid_f[type]], 0);
    }
    ...
    if (type == DNS_QTYPE_NS || type == DNS_QTYPE_CNAME) {
        ...
        char    *rdata_name = get_name_f(rdata, rdlength, payload, payload_len,
        ...
        if (rdlength <= 4) {
            ...
            char *rdata_name = get_name_f(rdata + 2, rdlength - 2, payload,
        ...
        if (rdlength >= 1 && (rdlength - 1) != *(uint8_t *) rdata) {
            log_warn("dnsf", "TXT record with wrong TXT len. Not processing.");
            fs_add_uint64(afs, "rdata_is_parsed", 0);
            fs_add_binary(afs, "rdata", rdlength, rdata, 0);
        } else if (rdlength < 1) {
            fs_add_uint64(afs, "rdata_is_parsed", 0);
        ...
        if (rdlength != 4) {
            log_warn("dnsf", "A record with IP of length %d. Not processing.",
                     rdlength);
            fs_add_uint64(afs, "rdata_is_parsed", 0);
            fs_add_binary(afs, "rdata", rdlength, rdata, 0);
        } else {
    ...
    *data_len = *data_len - bytes_consumed - sizeof(dns_answer_tail) - rdlength;
    log_trace("dnsf",
              "return success from process_response_answer_f, data_len: %d",
              *data_len);

    return 0;
}

static int load_question_from_str_f(const char *type_q_str) {
    ...
    while (1) {
        ...
        if (probe_q_delimiter_p == type_q_str ||
            ...
            log_error("dnsf", dnsf_usage_error);
            return EXIT_FAILURE;
        }

        if (index_questions_f >= num_questions_f) {
            log_error("dnsf", "less probes than questions configured. Add "
        ...
        if (label_type_f == DNS_LTYPE_STR) {
            domains_f[index_questions_f] =
                xmalloc(label_len_f + 1 + domain_len + 1);
            strncpy(domains_f[index_questions_f], label_f, label_len_f);
            domains_f[index_questions_f][label_len_f] = '.';
            strncpy(domains_f[index_questions_f] + label_len_f + 1,
                    probe_q_delimiter_p + 1, domain_len);
            domains_f[index_questions_f][label_len_f + 1 + domain_len] = '\0';
        } else {
            domains_f[index_questions_f] = xmalloc(domain_len + 1);
            strncpy(domains_f[index_questions_f], probe_q_delimiter_p + 1,
                    domain_len);
            domains_f[index_questions_f][domain_len] = '\0';
        }
        ...
        qtypes_f[index_questions_f] = qtype_str_to_code_f(strupr(qtype_str));
        if (!qtypes_f[index_questions_f]) {
            log_error("dnsf", "incorrect qtype supplied. %s", qtype_str);
            free(qtype_str);
            return EXIT_FAILURE;
        }
        free(qtype_str);
...
static int load_question_from_file_f(const char *file) {
    log_debug("dnsf", "load dns query domains from file");

    FILE *fp = fopen(file, "r");
    if (fp == NULL) {
        log_error("dnsf", "null dns domain file");
        return EXIT_FAILURE;
    }
    ...
    while (!feof(fp)) {
        ...
        pos = strchr(line, '\n');
        if (pos != NULL) *pos = '\0';
        if (load_question_from_str_f(line)) return EXIT_FAILURE;
    }

    return EXIT_SUCCESS;
}

int dns_random_bytes_f(char *dst, int len, const unsigned char *charset,
...
static int dnsf_global_init(struct state_conf *conf) {
    num_questions_f = conf->target_index_num;
    ...
    if (num_questions_f < 1) {
        log_fatal("dnsf", "invalid number of probes for the DNS module: %d",
                  num_questions_f);
    }

    // Setup the global structures
    dns_packets_f     = xmalloc(sizeof(char *) * num_questions_f);
    dns_packet_lens_f = xmalloc(sizeof(uint16_t) * num_questions_f);
    qname_lens_f      = xmalloc(sizeof(uint16_t) * num_questions_f);
    qnames_f          = xmalloc(sizeof(char *) * num_questions_f);
    qtypes_f          = xmalloc(sizeof(uint16_t) * num_questions_f);
    domains_f         = xmalloc(sizeof(char *) * num_questions_f);

    for (int i = 0; i < num_questions_f; i++) {
        domains_f[i] = (char *) default_domain_f;
        qtypes_f[i]  = default_qtype_f;
    }

    // This is xmap boilerplate. Why do I have to write this?
    dns_num_ports_f = conf->source_port_last - conf->source_port_first + 1;
    setup_qtype_str_map_f();
    ...
    if (conf->probe_args &&
        ...
        if (strncasecmp(conf->probe_args, "raw", 3) == 0) {
            label_type_f = DNS_LTYPE_RAW;
            log_debug("dnsf", "raw label prefix");
        } else if (strncasecmp(conf->probe_args, "time", 4) == 0) {
            label_type_f = DNS_LTYPE_TIME;
            log_debug("dnsf", "time label prefix");
        } else if (strncasecmp(conf->probe_args, "random", 6) == 0) {
            label_type_f = DNS_LTYPE_RANDOM;
            log_debug("dnsf", "random label prefix");
        } else if (strncasecmp(conf->probe_args, "str", 3) == 0) {
            label_type_f     = DNS_LTYPE_STR;
            conf->probe_args = c;
            ...
            label_len_f = c - conf->probe_args;
            label_f     = xmalloc(label_len_f);
            strncpy(label_f, conf->probe_args, label_len_f);
            ++c;
            log_debug("dnsf", "label prefix: %s, len: %d", label_f,
                      label_len_f);
        } else if (strncasecmp(conf->probe_args, "dst-ip", 6) == 0) {
            label_type_f = DNS_LTYPE_SRCIP;
            log_debug("dnsf", "dst-ip label prefix");
        } else {
            log_error("dnsf", dnsf_usage_error);
            return EXIT_FAILURE;
        }
        ...
        if (strncasecmp(conf->probe_args, "text", 4) == 0) {
            if (load_question_from_str_f(c)) return EXIT_FAILURE;
        } else if (strncasecmp(conf->probe_args, "file", 4) == 0) {
            if (load_question_from_file_f(c)) return EXIT_FAILURE;
        } else {
            log_error("dnsf", dnsf_usage_error);
            return EXIT_FAILURE;
        }
        ...
        if (index_questions_f < num_questions_f) {
            log_error("dnsf", "more probes than questions configured. Add "
                              "additional probes.");
            return EXIT_FAILURE;
        }
    }

    if (label_type_f == DNS_LTYPE_RAW || label_type_f == DNS_LTYPE_STR)
        return build_global_dns_packets_f(domains_f, num_questions_f);
    else
        return EXIT_SUCCESS;
}

static int dnsf_global_cleanup(UNUSED struct state_conf *xconf,
                               UNUSED struct state_send *xsend,
                               UNUSED struct state_recv *xrecv) {
    if (dns_packets_f) {
        for (int i = 0; i < num_questions_f; i++) {
            if (dns_packets_f[i]) {
                free(dns_packets_f[i]);
            }
        }
        free(dns_packets_f);
    }
    ...
    if (dns_packet_lens_f) {
        free(dns_packet_lens_f);
    }
...
int dnsf_thread_init(void *buf, macaddr_t *src, macaddr_t *gw, void **arg_ptr) {
    memset(buf, 0, MAX_PACKET_SIZE);

    // Setup assuming num_questions_f == 0
    struct ether_header *eth_header = (struct ether_header *) buf;
    make_eth_header(eth_header, src, gw);

    struct ip *ip_header = (struct ip *) (&eth_header[1]);
    uint16_t   ip_len =
        sizeof(struct ip) + sizeof(struct udphdr) + dns_packet_lens_f[0];
    make_ip_header(ip_header, IPPROTO_UDP, ip_len);

    struct udphdr *udp_header = (struct udphdr *) (&ip_header[1]);
    uint16_t       udp_len    = sizeof(struct udphdr) + dns_packet_lens_f[0];
    make_udp_header(udp_header, udp_len);

    char *payload             = (char *) (&udp_header[1]);
    module_dnsf.packet_length = sizeof(struct ether_header) +
                                sizeof(struct ip) + sizeof(struct udphdr) +
                                dns_packet_lens_f[0];
    assert(module_dnsf.packet_length <= MAX_PACKET_SIZE);

    memcpy(payload, dns_packets_f[0], dns_packet_lens_f[0]);
...
int dnsf_make_packet(void *buf, size_t *buf_len, ipaddr_n_t *dst_ip,
    ...
    port_h_t src_port = get_src_port(dns_num_ports_f, probe_num, validation);
    uint16_t dns_txid = get_dns_txid(validation);

    if (label_type_f == DNS_LTYPE_RAW || label_type_f == DNS_LTYPE_STR) {
        // For num_questions_f == 1, we handle this in per-thread init. Do less
        // work
        if (num_questions_f > 1) {
            uint16_t ip_len = sizeof(struct ip) + sizeof(struct udphdr) +
                              dns_packet_lens_f[index];
            make_ip_header(ip_header, IPPROTO_UDP, ip_len);

            uint16_t udp_len = sizeof(struct udphdr) + dns_packet_lens_f[index];
            make_udp_header(udp_header, udp_len);

            char *payload = (char *) (&udp_header[1]);
            *buf_len      = sizeof(struct ether_header) + sizeof(struct ip) +
                       sizeof(struct udphdr) + dns_packet_lens_f[index];

            assert(*buf_len <= MAX_PACKET_SIZE);

            memcpy(payload, dns_packets_f[index], dns_packet_lens_f[index]);
        }
        ...
        case DNS_LTYPE_RANDOM: {
            ...
            dns_random_bytes_f(new_label, 8, charset_alpha_lower_f, 26, aes);
            new_label[8] = '\0';
            break;
        }
        case DNS_LTYPE_SRCIP: {
            snprintf(new_label, new_label_max_len, "%u-%u-%u-%u-%u-%u-%u",
                     probe_num + 1, dst_ip[0], dst_ip[1], dst_ip[2], dst_ip[3],
                     src_port, dns_txid);
            new_label[strlen(new_label)] = '\0';
            break;
        }
        default:
            log_fatal("dnsf", dnsf_usage_error);
            return EXIT_FAILURE;
        }
        ...
        if (dns_packet_lens_f[index] > DNS_SEND_LEN) {
            log_fatal("dnsf", "DNS packet bigger (%d) than our limit (%d)",
                      dns_packet_lens_f[index], DNS_SEND_LEN);
            return EXIT_FAILURE;
        }
        ...
        char              *qname_p = dns_packets_f[index] + sizeof(dns_header);
        dns_question_tail *tail_p =
            (dns_question_tail *) (dns_packets_f[index] + sizeof(dns_header) +
                                   qname_lens_f[index]);
        ...
        // We have 1 question
        dns_header_p->qdcount = htons(1);
        memcpy(qname_p, qnames_f[index], qname_lens_f[index]);
        // Set the qtype to what we passed from args
        tail_p->qtype = htons(qtypes_f[index]);
        // Set the qclass to The Internet (TM) (R) (I hope you're happy
        // now Zakir)
        tail_p->qclass = htons(0x01);
        // MAGIC NUMBER. Let's be honest. This is only ever 1
...
void dnsf_print_packet(FILE *fp, void *packet) {
    ...
    fprintf(fp,
            ...
            question_name, qtype_strs_f[qtype_qtype_to_strid_f[tail_p->qtype]]);
    fprintf(fp, "------------------------------------------------------\n");

    free(question_name);
}

int dnsf_validate_packet(const struct ip *ip_hdr, uint32_t len,
    ...
    if (ip_hdr->ip_p == IPPROTO_UDP) {
        ...
        if (!check_dns_src_port(sport, dns_num_ports_f, validation)) {
            return PACKET_INVALID;
        }
...
void dnsf_process_packet(const u_char *packet, uint32_t len, fieldset_t *fs,
    ...
    if (ip_header->ip_p == IPPROTO_UDP) {
        ...
        for (int i = 0; i < ntohs(dns_header_p->qdcount) && !err; i++) {
            err = process_response_question_f(
        ...
        // packet prevent this.
        log_fatal("dnsf", "Die. This can only happen if you "
...
probe_module_t module_dnsf = {
    ...
    .port_args       = 1,
    .global_init     = &dnsf_global_init,
    .thread_init     = &dnsf_thread_init,
    .make_packet     = &dnsf_make_packet,
    .print_packet    = &dnsf_print_packet,
    .validate_packet = &dnsf_validate_packet,
    .process_packet  = &dnsf_process_packet,
    .close           = &dnsf_global_cleanup,
