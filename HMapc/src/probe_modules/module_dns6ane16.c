 ...
 *
 * Usage: xmap -p 53 --probe-module=dns6ane16 --probe-args="raw:text:A,qq.com"
...
// xmap boilerplate
probe_module_t module_dns6ane16;
static int     dns_num_ports_6ane16;

const char     default_domain_6ane16[] = "www.qq.com";
const uint16_t default_qtype_6ane16    = DNS_QTYPE_A;
const char    *dns6ane16_usage_error =
...
const unsigned char *charset_alpha_lower_6ane16 =
    (unsigned char *) "abcdefghijklmnopqrstuvwxyz";

static char    **dns_packets_6ane16;
static uint16_t *dns_packet_lens_6ane16; // Not including udp header
static uint16_t *qname_lens_6ane16;      // domain_len list
static char    **qnames_6ane16;          // domain list for query
static uint16_t *qtypes_6ane16;          // query_type list
static char    **domains_6ane16;         // domain strs
static int       num_questions_6ane16   = 0;
static int       index_questions_6ane16 = 0;

const char      default_option_qname_6ane16[]   = {0x00};
static int      default_option_qname_len_6ane16 = 1;
static uint16_t default_option_udpsize_6ane16   = 65535;
const char      default_option_rdata_6ane16[];
static int      default_option_rdata_len_6ane16 = 0;

/* Array of qtypes_6ane16 we support. Jumping through some hops (1 level of
 * indirection) so the per-packet processing time is fast. Keep this in sync
 * with: dns_qtype (.h) qtype_strid_to_qtype_6ane16 (below)
 * qtype_qtype_to_strid_6ane16 (below, and setup_qtype_str_map_6ane16())
 */
const char *qtype_strs_6ane16[] = {"A",    "NS",    "CNAME", "SOA",      "PTR",
                                   "MX",   "TXT",   "AAAA",  "RRSIG",    "ANY",
                                   "SIG",  "SRV",   "DS",    "DNSKEY",   "TLSA",
                                   "SVCB", "HTTPS", "CAA",   "HTTPSSVC", "OPT"};
const int   qtype_strs_len_6ane16 = 20;

const dns_qtype qtype_strid_to_qtype_6ane16[] = {
...
int8_t qtype_qtype_to_strid_6ane16[65536] = {BAD_QTYPE_VAL};

void setup_qtype_str_map_6ane16() {
    qtype_qtype_to_strid_6ane16[DNS_QTYPE_A]        = 0;
    qtype_qtype_to_strid_6ane16[DNS_QTYPE_NS]       = 1;
    qtype_qtype_to_strid_6ane16[DNS_QTYPE_CNAME]    = 2;
    qtype_qtype_to_strid_6ane16[DNS_QTYPE_SOA]      = 3;
    qtype_qtype_to_strid_6ane16[DNS_QTYPE_PTR]      = 4;
    qtype_qtype_to_strid_6ane16[DNS_QTYPE_MX]       = 5;
    qtype_qtype_to_strid_6ane16[DNS_QTYPE_TXT]      = 6;
    qtype_qtype_to_strid_6ane16[DNS_QTYPE_AAAA]     = 7;
    qtype_qtype_to_strid_6ane16[DNS_QTYPE_RRSIG]    = 8;
    qtype_qtype_to_strid_6ane16[DNS_QTYPE_ALL]      = 9;
    qtype_qtype_to_strid_6ane16[DNS_QTYPE_SIG]      = 10;
    qtype_qtype_to_strid_6ane16[DNS_QTYPE_SRV]      = 11;
    qtype_qtype_to_strid_6ane16[DNS_QTYPE_DS]       = 12;
    qtype_qtype_to_strid_6ane16[DNS_QTYPE_DNSKEY]   = 13;
    qtype_qtype_to_strid_6ane16[DNS_QTYPE_TLSA]     = 14;
    qtype_qtype_to_strid_6ane16[DNS_QTYPE_SVCB]     = 15;
    qtype_qtype_to_strid_6ane16[DNS_QTYPE_HTTPS]    = 16;
    qtype_qtype_to_strid_6ane16[DNS_QTYPE_CAA]      = 17;
    qtype_qtype_to_strid_6ane16[DNS_QTYPE_HTTPSSVC] = 18;
    qtype_qtype_to_strid_6ane16[DNS_QTYPE_OPT]      = 19;
}

static uint16_t qtype_str_to_code_6ane16(const char *str) {
    for (int i = 0; i < qtype_strs_len_6ane16; i++) {
        if (strcmp(qtype_strs_6ane16[i], str) == 0)
            return qtype_strid_to_qtype_6ane16[i];
    }

    return 0;
}

static char    *label_6ane16      = NULL;
static uint16_t label_len_6ane16  = 0;
static uint16_t label_type_6ane16 = DNS_LTYPE_RAW;
static uint16_t recursive_6ane16  = 1;

static uint16_t domain_to_qname_6ane16(char      **qname_handle,
                                       const char *domain) {
    if (domain[0] == '.') {
...
static int build_global_dns_packets_6ane16(char **domains, int num_domains) {
    for (int i = 0; i < num_domains; i++) {
        qname_lens_6ane16[i] =
            domain_to_qname_6ane16(&qnames_6ane16[i], domains[i]);
        if (domains[i] != (char *) default_domain_6ane16) {
            free(domains[i]);
        }
        dns_packet_lens_6ane16[i] =
            sizeof(dns_header) + qname_lens_6ane16[i] +
            sizeof(dns_question_tail) + default_option_qname_len_6ane16 +
            sizeof(dns_option_tail) + default_option_rdata_len_6ane16;
        if (dns_packet_lens_6ane16[i] > DNS_SEND_LEN) {
            log_fatal("dns6ane16", "DNS packet bigger (%d) than our limit (%d)",
                      dns_packet_lens_6ane16[i], DNS_SEND_LEN);
            return EXIT_FAILURE;
        }

        dns_packets_6ane16[i]           = xmalloc(dns_packet_lens_6ane16[i]);
        dns_header        *dns_header_p = (dns_header *) dns_packets_6ane16[i];
        char              *qname_p = dns_packets_6ane16[i] + sizeof(dns_header);
        dns_question_tail *tail_p =
            (dns_question_tail *) (dns_packets_6ane16[i] + sizeof(dns_header) +
                                   qname_lens_6ane16[i]);
        char *option_qname_p =
            (char *) (dns_packets_6ane16[i] + sizeof(dns_header) +
                      qname_lens_6ane16[i] + sizeof(dns_question_tail));
        dns_option_tail *option_tail_p =
            (dns_option_tail *) (dns_packets_6ane16[i] + sizeof(dns_header) +
                                 qname_lens_6ane16[i] +
                                 sizeof(dns_question_tail) +
                                 default_option_qname_len_6ane16);

        // All other header fields should be 0. Except id, which we set
        // per thread. Please recurse as needed.
        dns_header_p->rd = recursive_6ane16; // Is one bit. Don't need htons
        // We have 1 question
        dns_header_p->qdcount = htons(1);
        memcpy(qname_p, qnames_6ane16[i], qname_lens_6ane16[i]);
        // Set the qtype to what we passed from args
        tail_p->qtype = htons(qtypes_6ane16[i]);
        ...
        memcpy(option_qname_p, default_option_qname_6ane16,
               default_option_qname_len_6ane16);
        option_tail_p->type    = htons(DNS_QTYPE_OPT);
        option_tail_p->udpsize = htons(default_option_udpsize_6ane16);
    }

    return EXIT_SUCCESS;
}

static uint16_t get_name_helper_6ane16(const char *data, uint16_t data_len,
                                       const char *payload,
    ...
    if (recursion_level > MAX_LABEL_RECURSION) {
        log_trace("dns6ane16", "_get_name_helper OUT. ERR, MAX RECUSION");
        return 0;
    }
    ...
    while (data_len > 0) {
        ...
        // Is this a pointer?
        if (byte >= 0xc0) {
            log_trace("dns6ane16", "_get_name_helper, ptr encountered");
            ...
            log_trace("dns6ane16", "_get_name_helper. ptr offset 0x%x", offset);
            ...
            uint16_t rec_bytes_consumed = get_name_helper_6ane16(
            ...
            if (byte > name_len) {
                log_warn("dns6ane16", "Exceeded static name field allocation.");
                return 0;
            }
...
static char *get_name_6ane16(const char *data, uint16_t data_len,
                             const char *payload, uint16_t payload_len,
                             uint16_t *bytes_consumed) {
    log_trace("dns6ane16", "call to get_name_6ane16, data_len: %d", data_len);
    char *name      = xmalloc(MAX_NAME_LENGTH);
    *bytes_consumed = get_name_helper_6ane16(
    ...
    log_trace(
        "dns6ane16",
        "return success from get_name_6ane16, bytes_consumed: %d, string: %s",
        *bytes_consumed, name);

    return name;
}

static bool process_response_question_6ane16(char **data, uint16_t *data_len,
                                             const char *payload,
    ...
    uint16_t bytes_consumed = 0;
    char    *question_name  = get_name_6ane16(*data, *data_len, payload,
    ...
    if (qtype > MAX_QTYPE ||
        qtype_qtype_to_strid_6ane16[qtype] == BAD_QTYPE_VAL) {
        ...
        fs_add_string(
            qfs, "qtype_str",
            (char *) qtype_strs_6ane16[qtype_qtype_to_strid_6ane16[qtype]], 0);
    }
...
static bool process_response_answer_6ane16(char **data, uint16_t *data_len,
                                           const char *payload,
                                           uint16_t    payload_len,
                                           fieldset_t *list) {
    log_trace("dns6ane16",
              "call to process_response_answer_6ane16, data_len: %d",
              *data_len);
    // Payload is the start of the DNS packet, including header
    // data is handle to the start of this RR
    // data_len is a pointer to the how much total data we have to work
    // with. This is awful. I'm bad and should feel bad.
    uint16_t bytes_consumed = 0;
    char *answer_name = get_name_6ane16(*data, *data_len, payload, payload_len,
                                        &bytes_consumed);
    // Error.
    if (answer_name == NULL) {
        return 1;
    ...
    if (type > MAX_QTYPE ||
        qtype_qtype_to_strid_6ane16[type] == BAD_QTYPE_VAL) {
        ...
        fs_add_string(
            afs, "type_str",
            (char *) qtype_strs_6ane16[qtype_qtype_to_strid_6ane16[type]], 0);
    }
    ...
    if (type == DNS_QTYPE_NS || type == DNS_QTYPE_CNAME) {
        ...
        char    *rdata_name = get_name_6ane16(rdata, rdlength, payload,
        ...
        if (rdlength <= 4) {
            ...
            char *rdata_name =
                get_name_6ane16(rdata + 2, rdlength - 2, payload, payload_len,
    ...
    *data_len = *data_len - bytes_consumed - sizeof(dns_answer_tail) - rdlength;
    log_trace(
        "dns6ane16",
        "return success from process_response_answer_6ane16, data_len: %d",
        *data_len);

    return 0;
}

static int load_question_from_str_6ane16(const char *type_q_str) {
    ...
    while (1) {
        ...
        if (probe_q_delimiter_p == type_q_str ||
            ...
            log_error("dns6ane16", dns6ane16_usage_error);
            return EXIT_FAILURE;
        }

        if (index_questions_6ane16 >= num_questions_6ane16) {
            log_error("dns6ane16", "less probes than questions configured. Add "
        ...
        if (label_type_6ane16 == DNS_LTYPE_STR) {
            domains_6ane16[index_questions_6ane16] =
                xmalloc(label_len_6ane16 + 1 + domain_len + 1);
            strncpy(domains_6ane16[index_questions_6ane16], label_6ane16,
                    label_len_6ane16);
            domains_6ane16[index_questions_6ane16][label_len_6ane16] = '.';
            strncpy(domains_6ane16[index_questions_6ane16] + label_len_6ane16 +
                        1,
                    probe_q_delimiter_p + 1, domain_len);
            domains_6ane16[index_questions_6ane16]
                          [label_len_6ane16 + 1 + domain_len] = '\0';
        } else {
            domains_6ane16[index_questions_6ane16] = xmalloc(domain_len + 1);
            strncpy(domains_6ane16[index_questions_6ane16],
                    probe_q_delimiter_p + 1, domain_len);
            domains_6ane16[index_questions_6ane16][domain_len] = '\0';
        }
        ...
        qtypes_6ane16[index_questions_6ane16] =
            qtype_str_to_code_6ane16(strupr(qtype_str));
        if (!qtypes_6ane16[index_questions_6ane16]) {
            log_error("dns6ane16", "incorrect qtype supplied: %s", qtype_str);
            free(qtype_str);
            return EXIT_FAILURE;
        }
        free(qtype_str);
...
static int load_question_from_file_6ane16(const char *file) {
    log_debug("dns6ane16", "load dns query domains from file");

    FILE *fp = fopen(file, "r");
    if (fp == NULL) {
        log_error("dns6ane16", "null dns domain file");
        return EXIT_FAILURE;
    }
    ...
    while (!feof(fp)) {
        ...
        pos = strchr(line, '\n');
        if (pos != NULL) *pos = '\0';
        if (load_question_from_str_6ane16(line)) return EXIT_FAILURE;
    }

    return EXIT_SUCCESS;
}

int dns_random_bytes_6ane16(char *dst, int len, const unsigned char *charset,
...
static int dns6ane16_global_init(struct state_conf *conf) {
    num_questions_6ane16 = conf->target_index_num;

    if (!conf->probe_args) {
        conf->target_index_num = 1;
        num_questions_6ane16   = 1;
    }

    if (num_questions_6ane16 < 1) {
        log_fatal("dns6ane16",
                  "invalid number of probes for the DNS module: %d",
                  num_questions_6ane16);
    }

    // Setup the global structures
    dns_packets_6ane16     = xmalloc(sizeof(char *) * num_questions_6ane16);
    dns_packet_lens_6ane16 = xmalloc(sizeof(uint16_t) * num_questions_6ane16);
    qname_lens_6ane16      = xmalloc(sizeof(uint16_t) * num_questions_6ane16);
    qnames_6ane16          = xmalloc(sizeof(char *) * num_questions_6ane16);
    qtypes_6ane16          = xmalloc(sizeof(uint16_t) * num_questions_6ane16);
    domains_6ane16         = xmalloc(sizeof(char *) * num_questions_6ane16);

    for (int i = 0; i < num_questions_6ane16; i++) {
        domains_6ane16[i] = (char *) default_domain_6ane16;
        qtypes_6ane16[i]  = default_qtype_6ane16;
    }

    // This is xmap boilerplate. Why do I have to write this?
    dns_num_ports_6ane16 = conf->source_port_last - conf->source_port_first + 1;
    setup_qtype_str_map_6ane16();
    ...
    if (conf->probe_args &&
        ...
        if (strncasecmp(conf->probe_args, "raw", 3) == 0) {
            label_type_6ane16 = DNS_LTYPE_RAW;
            log_debug("dns6ane16", "raw label prefix");
        } else if (strncasecmp(conf->probe_args, "time", 4) == 0) {
            label_type_6ane16 = DNS_LTYPE_TIME;
            log_debug("dns6ane16", "time label prefix");
        } else if (strncasecmp(conf->probe_args, "random", 6) == 0) {
            label_type_6ane16 = DNS_LTYPE_RANDOM;
            log_debug("dns6ane16", "random label prefix");
        } else if (strncasecmp(conf->probe_args, "str", 3) == 0) {
            label_type_6ane16 = DNS_LTYPE_STR;
            conf->probe_args  = c;
            ...
            label_len_6ane16 = c - conf->probe_args;
            label_6ane16     = xmalloc(label_len_6ane16);
            strncpy(label_6ane16, conf->probe_args, label_len_6ane16);
            ++c;
            log_debug("dns6ane16", "label prefix: %s, len: %d", label_6ane16,
                      label_len_6ane16);
        } else if (strncasecmp(conf->probe_args, "dst-ip", 6) == 0) {
            label_type_6ane16 = DNS_LTYPE_SRCIP;
            log_debug("dns6ane16", "dst-ip label prefix");
        } else {
            log_error("dns6ane16", dns6ane16_usage_error);
            return EXIT_FAILURE;
        }
        ...
        if (strncasecmp(conf->probe_args, "text", 4) == 0) {
            if (load_question_from_str_6ane16(c)) return EXIT_FAILURE;
        } else if (strncasecmp(conf->probe_args, "file", 4) == 0) {
            if (load_question_from_file_6ane16(c)) return EXIT_FAILURE;
        } else {
            log_error("dns6ane16", dns6ane16_usage_error);
            return EXIT_FAILURE;
        }
        ...
        if (index_questions_6ane16 < num_questions_6ane16) {
            log_error("dns6ane16", "more probes than questions configured. Add "
                                   "additional probes.");
            return EXIT_FAILURE;
        }
    }

    if (label_type_6ane16 == DNS_LTYPE_RAW ||
        label_type_6ane16 == DNS_LTYPE_STR)
        return build_global_dns_packets_6ane16(domains_6ane16,
                                               num_questions_6ane16);
    else
        return EXIT_SUCCESS;
}

static int dns6ane16_global_cleanup(UNUSED struct state_conf *xconf,
                                    UNUSED struct state_send *xsend,
                                    UNUSED struct state_recv *xrecv) {
    if (dns_packets_6ane16) {
        for (int i = 0; i < num_questions_6ane16; i++) {
            if (dns_packets_6ane16[i]) {
                free(dns_packets_6ane16[i]);
            }
        }
        free(dns_packets_6ane16);
    }
    dns_packets_6ane16 = NULL;

    if (qnames_6ane16) {
        for (int i = 0; i < num_questions_6ane16; i++) {
            if (qnames_6ane16[i]) {
                free(qnames_6ane16[i]);
            }
        }
        free(qnames_6ane16);
    }
    ...
    if (dns_packet_lens_6ane16) {
        free(dns_packet_lens_6ane16);
    }

    if (qname_lens_6ane16) {
        free(qname_lens_6ane16);
    }
...
int dns6ane16_thread_init(void *buf, macaddr_t *src, macaddr_t *gw,
                          void **arg_ptr) {
    memset(buf, 0, MAX_PACKET_SIZE);

    // Setup assuming num_questions_6ane16 == 0
    struct ether_header *eth_header = (struct ether_header *) buf;
    make_eth6_header(eth_header, src, gw);

    struct ip6_hdr *ip6_header = (struct ip6_hdr *) (&eth_header[1]);
    uint16_t payload_len = sizeof(struct udphdr) + dns_packet_lens_6ane16[0];
    make_ip6_header(ip6_header, IPPROTO_UDP, payload_len);

    struct udphdr *udp6_header = (struct udphdr *) (&ip6_header[1]);
    uint16_t       udp_len = sizeof(struct udphdr) + dns_packet_lens_6ane16[0];
    make_udp_header(udp6_header, udp_len);

    char *payload = (char *) (&udp6_header[1]);
    module_dns6ane16.packet_length =
        sizeof(struct ether_header) + sizeof(struct ip6_hdr) +
        sizeof(struct udphdr) + dns_packet_lens_6ane16[0];
    assert(module_dns6ane16.packet_length <= MAX_PACKET_SIZE);

    memcpy(payload, dns_packets_6ane16[0], dns_packet_lens_6ane16[0]);
...
int dns6ane16_make_packet(void *buf, size_t *buf_len, ipaddr_n_t *src_ip,
    ...
    port_h_t src_port = get_src_port(dns_num_ports_6ane16, 0, validation);
    uint16_t dns_txid = get_dnsa_txid(validation, 0);

    if (label_type_6ane16 == DNS_LTYPE_RAW ||
        label_type_6ane16 == DNS_LTYPE_STR) {
        // For num_questions_6ane16 == 1, we handle this in per-thread init. Do
        // less work
        if (num_questions_6ane16 > 1) {
            uint16_t payload_len =
                sizeof(struct udphdr) + dns_packet_lens_6ane16[index];
            make_ip6_header(ip6_header, IPPROTO_UDP, payload_len);
            ...
            *buf_len = sizeof(struct ether_header) + sizeof(struct ip6_hdr) +
            ...
            memcpy(payload, dns_packets_6ane16[index],
                   dns_packet_lens_6ane16[index]);
        }
        ...
        case DNS_LTYPE_RANDOM: {
            ...
            dns_random_bytes_6ane16(new_label, 8, charset_alpha_lower_6ane16,
                                    26, aes);
            new_label[8] = '\0';
            break;
        }
        ...
        default:
            log_fatal("dns6ane16", dns6ane16_usage_error);
            return EXIT_FAILURE;
        }

        snprintf(new_domain, MAX_NAME_LENGTH, "%s-%s", new_label,
                 domains_6ane16[index]);

        // dns packet
        free(qnames_6ane16[index]);

        qname_lens_6ane16[index] =
            domain_to_qname_6ane16(&qnames_6ane16[index], new_domain);
        dns_packet_lens_6ane16[index] =
            sizeof(dns_header) + qname_lens_6ane16[index] +
            sizeof(dns_question_tail) + default_option_qname_len_6ane16 +
            sizeof(dns_option_tail) + default_option_rdata_len_6ane16;
        if (dns_packet_lens_6ane16[index] > DNS_SEND_LEN) {
            log_fatal("dns6ane16", "DNS packet bigger (%d) than our limit (%d)",
                      dns_packet_lens_6ane16[index], DNS_SEND_LEN);
            return EXIT_FAILURE;
        }

        free(dns_packets_6ane16[index]);

        dns_packets_6ane16[index] = xmalloc(dns_packet_lens_6ane16[index]);
        dns_header *dns_header_p  = (dns_header *) dns_packets_6ane16[index];
        char       *qname_p = dns_packets_6ane16[index] + sizeof(dns_header);
        dns_question_tail *tail_p =
            (dns_question_tail *) (dns_packets_6ane16[index] +
                                   sizeof(dns_header) +
                                   qname_lens_6ane16[index]);
        char *option_qname_p =
            (char *) (dns_packets_6ane16[index] + sizeof(dns_header) +
                      qname_lens_6ane16[index] + sizeof(dns_question_tail));
        dns_option_tail *option_tail_p =
            (dns_option_tail *) (dns_packets_6ane16[index] +
                                 sizeof(dns_header) + qname_lens_6ane16[index] +
                                 sizeof(dns_question_tail) +
                                 default_option_qname_len_6ane16);
        ...
        // We have 1 question
        dns_header_p->qdcount = htons(1);
        memcpy(qname_p, qnames_6ane16[index], qname_lens_6ane16[index]);
        // Set the qtype to what we passed from args
        tail_p->qtype = htons(qtypes_6ane16[index]);
        // Set the qclass to The Internet (TM) (R) (I hope you're happy
        // now Zakir)
        tail_p->qclass = htons(0x01);
        // MAGIC NUMBER. Let's be honest. This is only ever 1
...
void dns6ane16_print_packet(FILE *fp, void *packet) {
    ...
    uint16_t bytes_consumed = 0;
    char *question_name = get_name_6ane16(data, data_len, (char *) dns_header_p,
    ...
    dns_option_tail *option_tail_p =
        (dns_option_tail *) (option_qname_p + default_option_qname_len_6ane16);
    ...
    fprintf(fp,
            ...
            question_name,
            qtype_strs_6ane16[qtype_qtype_to_strid_6ane16[tail_p->qtype]],
...
int dns6ane16_validate_packet(const struct ip *ip_hdr, uint32_t len,
    ...
    if (ip6_header->ip6_nxt == IPPROTO_UDP) {
        ...
        if (!check_dns_src_port(sport, dns_num_ports_6ane16, validation)) {
            return PACKET_INVALID;
        }
...
void dns6ane16_process_packet(const u_char *packet, uint32_t len,
    ...
    if (ip6_header->ip6_nxt == IPPROTO_UDP) {
        ...
        for (int i = 0; i < ntohs(dns_header_p->qdcount) && !err; i++) {
            err = process_response_question_6ane16(
        ...
        for (int i = 0; i < ntohs(dns_header_p->ancount) && !err; i++) {
            err = process_response_answer_6ane16(
                &data, &data_len, (char *) dns_header_p, udp_len, list);
        }
        ...
        // packet prevent this.
        log_fatal("dns6ane16", "Die. This can only happen if you "
...
probe_module_t module_dns6ane16 = {
    ...
    .port_args       = 1,
    .global_init     = &dns6ane16_global_init,
    .thread_init     = &dns6ane16_thread_init,
    .make_packet     = &dns6ane16_make_packet,
    .print_packet    = &dns6ane16_print_packet,
    .validate_packet = &dns6ane16_validate_packet,
    .process_packet  = &dns6ane16_process_packet,
    .close           = &dns6ane16_global_cleanup,
