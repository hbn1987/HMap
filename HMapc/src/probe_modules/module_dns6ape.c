 ...
 *
 * Usage: xmap -p 53 --probe-module=dns6ape --probe-args="raw:text:A,qq.com"
...
// xmap boilerplate
probe_module_t module_dns6ape;
static int     dns_num_ports_6ape;

const char     default_domain_6ape[] = "www.qq.com";
const uint16_t default_qtype_6ape    = DNS_QTYPE_A;
const char    *dns6ape_usage_error =
...
const unsigned char *charset_alpha_lower_6ape =
    (unsigned char *) "abcdefghijklmnopqrstuvwxyz";

static char    **dns_packets_6ape;
static uint16_t *dns_packet_lens_6ape; // Not including udp header
static uint16_t *qname_lens_6ape;      // domain_len list
static char    **qnames_6ape;          // domain list for query
static uint16_t *qtypes_6ape;          // query_type list
static char    **domains_6ape;         // domain strs
static int       num_questions_6ape   = 0;
static int       index_questions_6ape = 0;

const char      default_option_qname_6ape[]   = {0x00};
static int      default_option_qname_len_6ape = 1;
static uint16_t default_option_udpsize_6ape   = 4096;
const char      default_option_rdata_6ape[];
static int      default_option_rdata_len_6ape = 0;

/* Array of qtypes_6ape we support. Jumping through some hops (1 level of
 * indirection) so the per-packet processing time is fast. Keep this in sync
 * with: dns_qtype (.h) qtype_strid_to_qtype_6ape (below)
...
int8_t qtype_qtype_to_strid_6ape[65536] = {BAD_QTYPE_VAL};

void setup_qtype_str_map_6ape() {
    qtype_qtype_to_strid_6ape[DNS_QTYPE_A]        = 0;
    qtype_qtype_to_strid_6ape[DNS_QTYPE_NS]       = 1;
    qtype_qtype_to_strid_6ape[DNS_QTYPE_CNAME]    = 2;
    qtype_qtype_to_strid_6ape[DNS_QTYPE_SOA]      = 3;
    qtype_qtype_to_strid_6ape[DNS_QTYPE_PTR]      = 4;
    qtype_qtype_to_strid_6ape[DNS_QTYPE_MX]       = 5;
    qtype_qtype_to_strid_6ape[DNS_QTYPE_TXT]      = 6;
    qtype_qtype_to_strid_6ape[DNS_QTYPE_AAAA]     = 7;
    qtype_qtype_to_strid_6ape[DNS_QTYPE_RRSIG]    = 8;
    qtype_qtype_to_strid_6ape[DNS_QTYPE_ALL]      = 9;
    qtype_qtype_to_strid_6ape[DNS_QTYPE_SIG]      = 10;
    qtype_qtype_to_strid_6ape[DNS_QTYPE_SRV]      = 11;
    qtype_qtype_to_strid_6ape[DNS_QTYPE_DS]       = 12;
    qtype_qtype_to_strid_6ape[DNS_QTYPE_DNSKEY]   = 13;
    qtype_qtype_to_strid_6ape[DNS_QTYPE_TLSA]     = 14;
    qtype_qtype_to_strid_6ape[DNS_QTYPE_SVCB]     = 15;
    qtype_qtype_to_strid_6ape[DNS_QTYPE_HTTPS]    = 16;
    qtype_qtype_to_strid_6ape[DNS_QTYPE_CAA]      = 17;
    qtype_qtype_to_strid_6ape[DNS_QTYPE_HTTPSSVC] = 18;
    qtype_qtype_to_strid_6ape[DNS_QTYPE_OPT]      = 19;
}

static uint16_t qtype_str_to_code_6ape(const char *str) {
    for (int i = 0; i < qtype_strs_len_6ape; i++) {
        if (strcmp(qtype_strs_6ape[i], str) == 0)
            return qtype_strid_to_qtype_6ape[i];
    }

    return 0;
}

static char    *label_6ape      = NULL;
static uint16_t label_len_6ape  = 0;
static uint16_t label_type_6ape = DNS_LTYPE_RAW;
static uint16_t recursive_6ape  = 1;

static uint16_t domain_to_qname_6ape(char **qname_handle, const char *domain) {
    if (domain[0] == '.') {
...
static int build_global_dns_packets_6ape(char **domains, int num_domains) {
    for (int i = 0; i < num_domains; i++) {
        qname_lens_6ape[i] = domain_to_qname_6ape(&qnames_6ape[i], domains[i]);
        if (domains[i] != (char *) default_domain_6ape) {
            free(domains[i]);
        }
        ...
        dns_question_tail *tail_p =
            (dns_question_tail *) (dns_packets_6ape[i] + sizeof(dns_header) +
                                   qname_lens_6ape[i]);
        char *option_qname_p =
            (char *) (dns_packets_6ape[i] + sizeof(dns_header) +
        ...
        dns_header_p->rd = recursive_6ape; // Is one bit. Don't need htons
        // We have 1 question
        dns_header_p->qdcount = htons(1);
        memcpy(qname_p, qnames_6ape[i], qname_lens_6ape[i]);
        // Set the qtype to what we passed from args
        tail_p->qtype = htons(qtypes_6ape[i]);
        ...
        memcpy(option_qname_p, default_option_qname_6ape,
               default_option_qname_len_6ape);
        option_tail_p->type    = htons(DNS_QTYPE_OPT);
        option_tail_p->udpsize = htons(default_option_udpsize_6ape);
    }
...
static uint16_t get_name_helper_6ape(const char *data, uint16_t data_len,
    ...
    if (recursion_level > MAX_LABEL_RECURSION) {
        log_trace("dns6ape", "_get_name_helper OUT. ERR, MAX RECUSION");
        return 0;
    }
    ...
    while (data_len > 0) {
        ...
        // Is this a pointer?
        if (byte >= 0xc0) {
            log_trace("dns6ape", "_get_name_helper, ptr encountered");
            ...
            log_trace("dns6ape", "_get_name_helper. ptr offset 0x%x", offset);
            ...
            uint16_t rec_bytes_consumed = get_name_helper_6ape(
            ...
            log_trace("dns6ape", "_get_name_helper, segment 0x%hx encountered",
                      byte);
            ...
            if (byte > name_len) {
                log_warn("dns6ape", "Exceeded static name field allocation.");
                return 0;
            }
...
static char *get_name_6ape(const char *data, uint16_t data_len,
    ...
    *bytes_consumed = get_name_helper_6ape(data, data_len, payload, payload_len,
    ...
    log_trace(
        "dns6ape",
        "return success from get_name_6ape, bytes_consumed: %d, string: %s",
        *bytes_consumed, name);
...
static bool process_response_question_6ape(char **data, uint16_t *data_len,
    ...
    uint16_t bytes_consumed = 0;
    char    *question_name =
        get_name_6ape(*data, *data_len, payload, payload_len, &bytes_consumed);
    // Error.
...
static bool process_response_answer_6ape(char **data, uint16_t *data_len,
    ...
    if (type > MAX_QTYPE || qtype_qtype_to_strid_6ape[type] == BAD_QTYPE_VAL) {
        fs_add_string(afs, "type_str", (char *) BAD_QTYPE_STR, 0);
    } else {
        // I've written worse things than this 3rd arg. But I want to be
        // fast.
        fs_add_string(afs, "type_str",
                      (char *) qtype_strs_6ape[qtype_qtype_to_strid_6ape[type]],
                      0);
    }
    ...
    *data_len = *data_len - bytes_consumed - sizeof(dns_answer_tail) - rdlength;
    log_trace("dns6ape",
              "return success from process_response_answer_6ape, data_len: %d",
              *data_len);

    return 0;
}

static int load_question_from_str_6ape(const char *type_q_str) {
    ...
    while (1) {
        ...
        if (probe_q_delimiter_p == type_q_str ||
            ...
            log_error("dns6ape", dns6ape_usage_error);
            return EXIT_FAILURE;
        }
        ...
        if (label_type_6ape == DNS_LTYPE_STR) {
            ...
            strncpy(domains_6ape[index_questions_6ape], probe_q_delimiter_p + 1,
                    domain_len);
            domains_6ape[index_questions_6ape][domain_len] = '\0';
        }
        ...
        qtypes_6ape[index_questions_6ape] =
            qtype_str_to_code_6ape(strupr(qtype_str));
        if (!qtypes_6ape[index_questions_6ape]) {
            log_error("dns6ape", "incorrect qtype supplied: %s", qtype_str);
            free(qtype_str);
            return EXIT_FAILURE;
        }
        free(qtype_str);

        index_questions_6ape++;
        if (probe_arg_delimiter_p)
...
static int load_question_from_file_6ape(const char *file) {
    log_debug("dns6ape", "load dns query domains from file");

    FILE *fp = fopen(file, "r");
    if (fp == NULL) {
        log_error("dns6ape", "null dns domain file");
        return EXIT_FAILURE;
    }
    ...
    while (!feof(fp)) {
        ...
        pos = strchr(line, '\n');
        if (pos != NULL) *pos = '\0';
        if (load_question_from_str_6ape(line)) return EXIT_FAILURE;
    }
...
static int dns6ape_global_init(struct state_conf *conf) {
    num_questions_6ape = conf->target_index_num;
    ...
    if (num_questions_6ape < 1) {
        log_fatal("dns6ape", "invalid number of probes for the DNS module: %d",
                  num_questions_6ape);
    }

    // Setup the global structures
    dns_packets_6ape     = xmalloc(sizeof(char *) * num_questions_6ape);
    dns_packet_lens_6ape = xmalloc(sizeof(uint16_t) * num_questions_6ape);
    qname_lens_6ape      = xmalloc(sizeof(uint16_t) * num_questions_6ape);
    qnames_6ape          = xmalloc(sizeof(char *) * num_questions_6ape);
    qtypes_6ape          = xmalloc(sizeof(uint16_t) * num_questions_6ape);
    domains_6ape         = xmalloc(sizeof(char *) * num_questions_6ape);

    for (int i = 0; i < num_questions_6ape; i++) {
        domains_6ape[i] = (char *) default_domain_6ape;
        qtypes_6ape[i]  = default_qtype_6ape;
    }

    // This is xmap boilerplate. Why do I have to write this?
    dns_num_ports_6ape = conf->source_port_last - conf->source_port_first + 1;
    setup_qtype_str_map_6ape();
    ...
    if (conf->probe_args &&
        ...
        if (strncasecmp(conf->probe_args, "raw", 3) == 0) {
            label_type_6ape = DNS_LTYPE_RAW;
            log_debug("dns6ape", "raw label prefix");
        } else if (strncasecmp(conf->probe_args, "time", 4) == 0) {
            label_type_6ape = DNS_LTYPE_TIME;
            log_debug("dns6ape", "time label prefix");
            ...
            label_len_6ape = c - conf->probe_args;
            label_6ape     = xmalloc(label_len_6ape);
            strncpy(label_6ape, conf->probe_args, label_len_6ape);
            ++c;
            log_debug("dns6ape", "label prefix: %s, len: %d", label_6ape,
                      label_len_6ape);
        } else if (strncasecmp(conf->probe_args, "dst-ip", 6) == 0) {
            label_type_6ape = DNS_LTYPE_SRCIP;
            log_debug("dns6ape", "dst-ip label prefix");
        } else {
            log_error("dns6ape", dns6ape_usage_error);
            return EXIT_FAILURE;
        }
        ...
        if (strncasecmp(conf->probe_args, "text", 4) == 0) {
            if (load_question_from_str_6ape(c)) return EXIT_FAILURE;
        } else if (strncasecmp(conf->probe_args, "file", 4) == 0) {
            if (load_question_from_file_6ape(c)) return EXIT_FAILURE;
        } else {
            log_error("dns6ape", dns6ape_usage_error);
            return EXIT_FAILURE;
        }
    ...
    if (label_type_6ape == DNS_LTYPE_RAW || label_type_6ape == DNS_LTYPE_STR)
        return build_global_dns_packets_6ape(domains_6ape, num_questions_6ape);
    else
        return EXIT_SUCCESS;
}
...
static int dns6ape_global_cleanup(UNUSED struct state_conf *xconf,
    ...
    if (dns_packets_6ape) {
        for (int i = 0; i < num_questions_6ape; i++) {
            if (dns_packets_6ape[i]) {
                free(dns_packets_6ape[i]);
            }
        }
        free(dns_packets_6ape);
    }
    dns_packets_6ape = NULL;
    ...
    if (dns_packet_lens_6ape) {
        free(dns_packet_lens_6ape);
    }

    if (qname_lens_6ape) {
        free(qname_lens_6ape);
    }
...
int dns6ape_thread_init(void *buf, macaddr_t *src, macaddr_t *gw,
    ...
    module_dns6ape.packet_length =
        sizeof(struct ether_header) + sizeof(struct ip6_hdr) +
        sizeof(struct udphdr) + dns_packet_lens_6ape[0];
    assert(module_dns6ape.packet_length <= MAX_PACKET_SIZE);

    memcpy(payload, dns_packets_6ape[0], dns_packet_lens_6ape[0]);
...
int dns6ape_make_packet(void *buf, size_t *buf_len, ipaddr_n_t *src_ip,
    ...
    port_h_t src_port = get_src_port(dns_num_ports_6ape, probe_num, validation);
    uint16_t dns_txid = get_dnsa_txid(validation, 0);

    if (label_type_6ape == DNS_LTYPE_RAW || label_type_6ape == DNS_LTYPE_STR) {
        // For num_questions_6ape == 1, we handle this in per-thread init. Do
        // less work
        if (num_questions_6ape > 1) {
            uint16_t payload_len =
                sizeof(struct udphdr) + dns_packet_lens_6ape[index];
            make_ip6_header(ip6_header, IPPROTO_UDP, payload_len);
        ...
        default:
            log_fatal("dns6ape", dns6ape_usage_error);
            return EXIT_FAILURE;
        }
        ...
        if (dns_packet_lens_6ape[index] > DNS_SEND_LEN) {
            log_fatal("dns6ape", "DNS packet bigger (%d) than our limit (%d)",
                      dns_packet_lens_6ape[index], DNS_SEND_LEN);
            return EXIT_FAILURE;
        }
        ...
        dns_option_tail *option_tail_p =
            (dns_option_tail *) (dns_packets_6ape[index] + sizeof(dns_header) +
                                 qname_lens_6ape[index] +
        ...
        // We have 1 question
        dns_header_p->qdcount = htons(1);
        memcpy(qname_p, qnames_6ape[index], qname_lens_6ape[index]);
        // Set the qtype to what we passed from args
        tail_p->qtype = htons(qtypes_6ape[index]);
        // Set the qclass to The Internet (TM) (R) (I hope you're happy
        // now Zakir)
        tail_p->qclass = htons(0x01);
        // MAGIC NUMBER. Let's be honest. This is only ever 1
        ...
        uint16_t udp_len = sizeof(struct udphdr) + dns_packet_lens_6ape[index];
        make_udp_header(udp_header, udp_len);
...
void dns6ape_print_packet(FILE *fp, void *packet) {
    ...
    dns_option_tail *option_tail_p =
        (dns_option_tail *) (option_qname_p + default_option_qname_len_6ape);
    ...
    fprintf(fp,
            ...
            question_name,
            qtype_strs_6ape[qtype_qtype_to_strid_6ape[tail_p->qtype]],
...
int dns6ape_validate_packet(const struct ip *ip_hdr, uint32_t len,
    ...
    if (ip6_header->ip6_nxt == IPPROTO_UDP) {
        ...
        if (!check_dns_src_port(sport, dns_num_ports_6ape, validation)) {
            return PACKET_INVALID;
        }
...
void dns6ape_process_packet(const u_char *packet, uint32_t len, fieldset_t *fs,
    ...
    if (ip6_header->ip6_nxt == IPPROTO_UDP) {
        ...
        for (int i = 0; i < ntohs(dns_header_p->qdcount) && !err; i++) {
            err = process_response_question_6ape(
        ...
        for (int i = 0; i < ntohs(dns_header_p->ancount) && !err; i++) {
            err = process_response_answer_6ape(
                &data, &data_len, (char *) dns_header_p, udp_len, list);
        }
...
probe_module_t module_dns6ape = {
    ...
    .port_args       = 1,
    .global_init     = &dns6ape_global_init,
    .thread_init     = &dns6ape_thread_init,
    .make_packet     = &dns6ape_make_packet,
    .print_packet    = &dns6ape_print_packet,
    .validate_packet = &dns6ape_validate_packet,
    .process_packet  = &dns6ape_process_packet,
    .close           = &dns6ape_global_cleanup,
