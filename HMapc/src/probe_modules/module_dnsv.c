 ...
 *
 * Usage: xmap -p 53 --probe-module=dnsv --probe-args="raw:text:A,qq.com"
...
// xmap boilerplate
probe_module_t module_dnsv;
static int     dns_num_ports_v;

const char     default_domain_v[] = "www.qq.com";
const uint16_t default_qtype_v    = DNS_QTYPE_A;
const char    *dnsv_usage_error =
...
const unsigned char *charset_alpha_lower_v =
    (unsigned char *) "abcdefghijklmnopqrstuvwxyz";

static char    **dns_packets_v;
static uint16_t *dns_packet_lens_v; // Not including udp header
static uint16_t *qname_lens_v;      // domain_len list
static char    **qnames_v;          // domain list for query
static uint16_t *qtypes_v;          // query_type list
static char    **domains_v;         // domain strs
static int       num_questions_v   = 0;
static int       index_questions_v = 0;

/* Array of qtypes_v we support. Jumping through some hops (1 level of
 * indirection) so the per-packet processing time is fast. Keep this in sync
 * with: dns_qtype (.h) qtype_strid_to_qtype_v (below) qtype_qtype_to_strid_v
...
int8_t qtype_qtype_to_strid_v[256] = {BAD_QTYPE_VAL};

void setup_qtype_str_map_v() {
    qtype_qtype_to_strid_v[DNS_QTYPE_A]     = 0;
    qtype_qtype_to_strid_v[DNS_QTYPE_NS]    = 1;
    qtype_qtype_to_strid_v[DNS_QTYPE_CNAME] = 2;
    qtype_qtype_to_strid_v[DNS_QTYPE_SOA]   = 3;
    qtype_qtype_to_strid_v[DNS_QTYPE_PTR]   = 4;
    qtype_qtype_to_strid_v[DNS_QTYPE_MX]    = 5;
    qtype_qtype_to_strid_v[DNS_QTYPE_TXT]   = 6;
    qtype_qtype_to_strid_v[DNS_QTYPE_AAAA]  = 7;
    qtype_qtype_to_strid_v[DNS_QTYPE_RRSIG] = 8;
    qtype_qtype_to_strid_v[DNS_QTYPE_ALL]   = 9;
}

static uint16_t qtype_str_to_code_v(const char *str) {
    for (int i = 0; i < qtype_strs_len_v; i++) {
        if (strcmp(qtype_strs_v[i], str) == 0) return qtype_strid_to_qtype_v[i];
    }

    return 0;
}

static char    *label_v      = NULL;
static uint16_t label_len_v  = 0;
static uint16_t label_type_v = DNS_LTYPE_RAW;
static uint16_t recursive_v  = 1;

static uint16_t domain_to_qname_v(char **qname_handle, const char *domain) {
    if (domain[0] == '.') {
...
static int build_global_dns_packets_v(char **domains, int num_domains) {
    for (int i = 0; i < num_domains; i++) {
        qname_lens_v[i] = domain_to_qname_v(&qnames_v[i], domains[i]);
        if (domains[i] != (char *) default_domain_v) {
            free(domains[i]);
        }
        dns_packet_lens_v[i] =
            sizeof(dns_header) + qname_lens_v[i] + sizeof(dns_question_tail);
        if (dns_packet_lens_v[i] > DNS_SEND_LEN) {
            log_fatal("dnsv", "DNS packet bigger (%d) than our limit (%d)",
                      dns_packet_lens_v[i], DNS_SEND_LEN);
            return EXIT_FAILURE;
        }

        dns_packets_v[i]                = xmalloc(dns_packet_lens_v[i]);
        dns_header        *dns_header_p = (dns_header *) dns_packets_v[i];
        char              *qname_p      = dns_packets_v[i] + sizeof(dns_header);
        dns_question_tail *tail_p =
            (dns_question_tail *) (dns_packets_v[i] + sizeof(dns_header) +
                                   qname_lens_v[i]);

        // All other header fields should be 0. Except id, which we set
        // per thread. Please recurse as needed.
        dns_header_p->rd = recursive_v; // Is one bit. Don't need htons
        // We have 1 question
        dns_header_p->qdcount = htons(1);
        memcpy(qname_p, qnames_v[i], qname_lens_v[i]);
        // Set the qtype to what we passed from args
        tail_p->qtype = htons(qtypes_v[i]);
        // Set the qclass to The Chaos (CH) (R) (I hope you're happy
        // now Zakir)
        tail_p->qclass = htons(0x03);
...
static uint16_t get_name_helper_v(const char *data, uint16_t data_len,
    ...
    if (recursion_level > MAX_LABEL_RECURSION) {
        log_trace("dnsv", "_get_name_helper OUT. ERR, MAX RECUSION");
        return 0;
    }
    ...
    while (data_len > 0) {
        ...
        // Is this a pointer?
        if (byte >= 0xc0) {
            log_trace("dnsv", "_get_name_helper, ptr encountered");
            // Do we have enough bytes to check ahead?
            if (data_len < 2) {
                log_trace("dnsv", "_get_name_helper OUT. ptr byte encountered. "
                                  "No offset. ERR.");
                return 0;
            }
            ...
            log_trace("dnsv", "_get_name_helper. ptr offset 0x%x", offset);
            ...
            if (recursion_level > 0 || bytes_consumed > 0) {
                ...
                if (name_len < 1) {
                    log_warn("dnsv", "Exceeded static name field allocation.");
                    return 0;
                }
            ...
            uint16_t rec_bytes_consumed = get_name_helper_v(
            ...
            if (rec_bytes_consumed == 0) {
                log_trace("dnsv", "_get_name_helper OUT. rec level %d failed",
                          recursion_level);
                return 0;
            } else {
                bytes_consumed += 2;
            ...
            log_trace("dnsv", "_get_name_helper, segment 0x%hx encountered",
                      byte);
            ...
            // null byte too)?
            if ((byte + 1) > data_len) {
                log_trace("dnsv", "_get_name_helper OUT. ERR. Not enough data "
                                  "for segment %hd");
                return 0;
            }
...
static char *get_name_v(const char *data, uint16_t data_len,
                        const char *payload, uint16_t payload_len,
                        uint16_t *bytes_consumed) {
    log_trace("dnsv", "call to get_name_v, data_len: %d", data_len);
    char *name      = xmalloc(MAX_NAME_LENGTH);
    *bytes_consumed = get_name_helper_v(data, data_len, payload, payload_len,
    ...
    log_trace("dnsv",
              "return success from get_name_v, bytes_consumed: %d, string: %s",
              *bytes_consumed, name);

    return name;
}

static bool process_response_question_v(char **data, uint16_t *data_len,
                                        const char *payload,
    ...
    uint16_t bytes_consumed = 0;
    char    *question_name =
        get_name_v(*data, *data_len, payload, payload_len, &bytes_consumed);
    // Error.
    ...
    if (qtype > MAX_QTYPE || qtype_qtype_to_strid_v[qtype] == BAD_QTYPE_VAL) {
        ...
        fs_add_string(qfs, "qtype_str",
                      (char *) qtype_strs_v[qtype_qtype_to_strid_v[qtype]], 0);
    }
...
static bool process_response_answer_v(char **data, uint16_t *data_len,
                                      const char *payload, uint16_t payload_len,
                                      fieldset_t *list) {
    log_trace("dnsv", "call to process_response_answer_v, data_len: %d",
              *data_len);
    // Payload is the start of the DNS packet, including header
    // data is handle to the start of this RR
    // data_len is a pointer to the how much total data we have to work
    ...
    if (type > MAX_QTYPE || qtype_qtype_to_strid_v[type] == BAD_QTYPE_VAL) {
        fs_add_string(afs, "type_str", (char *) BAD_QTYPE_STR, 0);
    } else {
        // I've written worse things than this 3rd arg. But I want to be
        // fast.
        fs_add_string(afs, "type_str",
                      (char *) qtype_strs_v[qtype_qtype_to_strid_v[type]], 0);
    }
    ...
    if (type == DNS_QTYPE_NS || type == DNS_QTYPE_CNAME) {
        ...
        char    *rdata_name = get_name_v(rdata, rdlength, payload, payload_len,
        ...
        if (rdlength <= 4) {
            ...
            char *rdata_name = get_name_v(rdata + 2, rdlength - 2, payload,
        ...
        if (rdlength >= 1 && (rdlength - 1) != *(uint8_t *) rdata) {
            log_warn("dnsv", "TXT record with wrong TXT len. Not processing.");
            fs_add_uint64(afs, "rdata_is_parsed", 0);
            fs_add_binary(afs, "rdata", rdlength, rdata, 0);
        } else if (rdlength < 1) {
            fs_add_uint64(afs, "rdata_is_parsed", 0);
        ...
        if (rdlength != 4) {
            log_warn("dnsv", "A record with IP of length %d. Not processing.",
                     rdlength);
            fs_add_uint64(afs, "rdata_is_parsed", 0);
            fs_add_binary(afs, "rdata", rdlength, rdata, 0);
        } else {
    ...
    *data_len = *data_len - bytes_consumed - sizeof(dns_answer_tail) - rdlength;
    log_trace("dnsv",
              "return success from process_response_answer_v, data_len: %d",
              *data_len);

    return 0;
}

static int load_question_from_str_v(const char *type_q_str) {
    ...
    while (1) {
        ...
        if (probe_q_delimiter_p == type_q_str ||
            ...
            log_error("dnsv", dnsv_usage_error);
            return EXIT_FAILURE;
        }

        if (index_questions_v >= num_questions_v) {
            log_error("dnsv", "less probes than questions configured. Add "
        ...
        if (label_type_v == DNS_LTYPE_STR) {
            domains_v[index_questions_v] =
                xmalloc(label_len_v + 1 + domain_len + 1);
            strncpy(domains_v[index_questions_v], label_v, label_len_v);
            domains_v[index_questions_v][label_len_v] = '.';
            strncpy(domains_v[index_questions_v] + label_len_v + 1,
                    probe_q_delimiter_p + 1, domain_len);
            domains_v[index_questions_v][label_len_v + 1 + domain_len] = '\0';
        } else {
            domains_v[index_questions_v] = xmalloc(domain_len + 1);
            strncpy(domains_v[index_questions_v], probe_q_delimiter_p + 1,
                    domain_len);
            domains_v[index_questions_v][domain_len] = '\0';
        }
        ...
        qtypes_v[index_questions_v] = qtype_str_to_code_v(strupr(qtype_str));
        if (!qtypes_v[index_questions_v]) {
            log_error("dnsv", "incorrect qtype supplied. %s", qtype_str);
            free(qtype_str);
            return EXIT_FAILURE;
        }
        free(qtype_str);
...
static int load_question_from_file_v(const char *file) {
    log_debug("dnsv", "load dns query domains from file");

    FILE *fp = fopen(file, "r");
    if (fp == NULL) {
        log_error("dnsv", "null dns domain file");
        return EXIT_FAILURE;
    }
    ...
    while (!feof(fp)) {
        ...
        pos = strchr(line, '\n');
        if (pos != NULL) *pos = '\0';
        if (load_question_from_str_v(line)) return EXIT_FAILURE;
    }

    return EXIT_SUCCESS;
}

int dns_random_bytes_v(char *dst, int len, const unsigned char *charset,
...
static int dnsv_global_init(struct state_conf *conf) {
    num_questions_v = conf->target_index_num;
    ...
    if (num_questions_v < 1) {
        log_fatal("dnsv", "invalid number of probes for the DNS module: %d",
                  num_questions_v);
    }

    // Setup the global structures
    dns_packets_v     = xmalloc(sizeof(char *) * num_questions_v);
    dns_packet_lens_v = xmalloc(sizeof(uint16_t) * num_questions_v);
    qname_lens_v      = xmalloc(sizeof(uint16_t) * num_questions_v);
    qnames_v          = xmalloc(sizeof(char *) * num_questions_v);
    qtypes_v          = xmalloc(sizeof(uint16_t) * num_questions_v);
    domains_v         = xmalloc(sizeof(char *) * num_questions_v);

    for (int i = 0; i < num_questions_v; i++) {
        domains_v[i] = (char *) default_domain_v;
        qtypes_v[i]  = default_qtype_v;
    }

    // This is xmap boilerplate. Why do I have to write this?
    dns_num_ports_v = conf->source_port_last - conf->source_port_first + 1;
    setup_qtype_str_map_v();
    ...
    if (conf->probe_args &&
        ...
        if (strncasecmp(conf->probe_args, "raw", 3) == 0) {
            label_type_v = DNS_LTYPE_RAW;
            log_debug("dnsv", "raw label prefix");
        } else if (strncasecmp(conf->probe_args, "time", 4) == 0) {
            label_type_v = DNS_LTYPE_TIME;
            log_debug("dnsv", "time label prefix");
        } else if (strncasecmp(conf->probe_args, "random", 6) == 0) {
            label_type_v = DNS_LTYPE_RANDOM;
            log_debug("dnsv", "random label prefix");
        } else if (strncasecmp(conf->probe_args, "str", 3) == 0) {
            label_type_v     = DNS_LTYPE_STR;
            conf->probe_args = c;
            ...
            label_len_v = c - conf->probe_args;
            label_v     = xmalloc(label_len_v);
            strncpy(label_v, conf->probe_args, label_len_v);
            ++c;
            log_debug("dnsv", "label prefix: %s, len: %d", label_v,
                      label_len_v);
        } else if (strncasecmp(conf->probe_args, "dst-ip", 6) == 0) {
            label_type_v = DNS_LTYPE_SRCIP;
            log_debug("dnsv", "dst-ip label prefix");
        } else {
            log_error("dnsv", dnsv_usage_error);
            return EXIT_FAILURE;
        }
        ...
        if (strncasecmp(conf->probe_args, "text", 4) == 0) {
            if (load_question_from_str_v(c)) return EXIT_FAILURE;
        } else if (strncasecmp(conf->probe_args, "file", 4) == 0) {
            if (load_question_from_file_v(c)) return EXIT_FAILURE;
        } else {
            log_error("dnsv", dnsv_usage_error);
            return EXIT_FAILURE;
        }
        ...
        if (index_questions_v < num_questions_v) {
            log_error("dnsv", "more probes than questions configured. Add "
                              "additional probes.");
            return EXIT_FAILURE;
        }
    }

    if (label_type_v == DNS_LTYPE_RAW || label_type_v == DNS_LTYPE_STR)
        return build_global_dns_packets_v(domains_v, num_questions_v);
    else
        return EXIT_SUCCESS;
}

static int dnsv_global_cleanup(UNUSED struct state_conf *xconf,
                               UNUSED struct state_send *xsend,
                               UNUSED struct state_recv *xrecv) {
    if (dns_packets_v) {
        for (int i = 0; i < num_questions_v; i++) {
            if (dns_packets_v[i]) {
                free(dns_packets_v[i]);
            }
        }
        free(dns_packets_v);
    }
    ...
    if (dns_packet_lens_v) {
        free(dns_packet_lens_v);
    }
...
int dnsv_thread_init(void *buf, macaddr_t *src, macaddr_t *gw, void **arg_ptr) {
    memset(buf, 0, MAX_PACKET_SIZE);

    // Setup assuming num_questions_v == 0
    struct ether_header *eth_header = (struct ether_header *) buf;
    make_eth_header(eth_header, src, gw);

    struct ip *ip_header = (struct ip *) (&eth_header[1]);
    uint16_t   ip_len =
        sizeof(struct ip) + sizeof(struct udphdr) + dns_packet_lens_v[0];
    make_ip_header(ip_header, IPPROTO_UDP, ip_len);

    struct udphdr *udp_header = (struct udphdr *) (&ip_header[1]);
    uint16_t       udp_len    = sizeof(struct udphdr) + dns_packet_lens_v[0];
    make_udp_header(udp_header, udp_len);

    char *payload             = (char *) (&udp_header[1]);
    module_dnsv.packet_length = sizeof(struct ether_header) +
                                sizeof(struct ip) + sizeof(struct udphdr) +
                                dns_packet_lens_v[0];
    assert(module_dnsv.packet_length <= MAX_PACKET_SIZE);

    memcpy(payload, dns_packets_v[0], dns_packet_lens_v[0]);
...
int dnsv_make_packet(void *buf, size_t *buf_len, ipaddr_n_t *src_ip,
    ...
    port_h_t src_port = get_src_port(dns_num_ports_v, probe_num, validation);
    uint16_t dns_txid = get_dns_txid(validation);

    if (label_type_v == DNS_LTYPE_RAW || label_type_v == DNS_LTYPE_STR) {
        // For num_questions_v == 1, we handle this in per-thread init. Do less
        // work
        if (num_questions_v > 1) {
            uint16_t ip_len = sizeof(struct ip) + sizeof(struct udphdr) +
                              dns_packet_lens_v[index];
            make_ip_header(ip_header, IPPROTO_UDP, ip_len);

            uint16_t udp_len = sizeof(struct udphdr) + dns_packet_lens_v[index];
            make_udp_header(udp_header, udp_len);

            char *payload = (char *) (&udp_header[1]);
            *buf_len      = sizeof(struct ether_header) + sizeof(struct ip) +
                       sizeof(struct udphdr) + dns_packet_lens_v[index];

            assert(*buf_len <= MAX_PACKET_SIZE);

            memcpy(payload, dns_packets_v[index], dns_packet_lens_v[index]);
        }
        ...
        case DNS_LTYPE_RANDOM: {
            ...
            dns_random_bytes_v(new_label, 8, charset_alpha_lower_v, 26, aes);
            new_label[8] = '\0';
            break;
        }
        ...
        default:
            log_fatal("dnsv", dnsv_usage_error);
            return EXIT_FAILURE;
        }
        ...
        if (dns_packet_lens_v[index] > DNS_SEND_LEN) {
            log_fatal("dnsv", "DNS packet bigger (%d) than our limit (%d)",
                      dns_packet_lens_v[index], DNS_SEND_LEN);
            return EXIT_FAILURE;
        }
        ...
        char              *qname_p = dns_packets_v[index] + sizeof(dns_header);
        dns_question_tail *tail_p =
            (dns_question_tail *) (dns_packets_v[index] + sizeof(dns_header) +
                                   qname_lens_v[index]);
        ...
        // We have 1 question
        dns_header_p->qdcount = htons(1);
        memcpy(qname_p, qnames_v[index], qname_lens_v[index]);
        // Set the qtype to what we passed from args
        tail_p->qtype = htons(qtypes_v[index]);
        // Set the qclass to The Chaos (CH) (R) (I hope you're happy
        // now Zakir)
        tail_p->qclass = htons(0x03);
        // MAGIC NUMBER. Let's be honest. This is only ever 1
...
void dnsv_print_packet(FILE *fp, void *packet) {
    ...
    fprintf(fp,
            ...
            question_name, qtype_strs_v[qtype_qtype_to_strid_v[tail_p->qtype]]);
    fprintf(fp, "------------------------------------------------------\n");

    free(question_name);
}

int dnsv_validate_packet(const struct ip *ip_hdr, uint32_t len,
    ...
    if (ip_hdr->ip_p == IPPROTO_UDP) {
        ...
        if (!check_dns_src_port(sport, dns_num_ports_v, validation)) {
            return PACKET_INVALID;
        }
...
void dnsv_process_packet(const u_char *packet, uint32_t len, fieldset_t *fs,
                         UNUSED struct timespec ts) {
    struct ip *ip_header = (struct ip *) &packet[sizeof(struct ether_header)];
    //    log_info("dnsv", "IP=%s",
    //    inet_in2constr((uint8_t*)&(ip_header->ip_src.s_addr),
    //    xconf.ipv46_flag));
    ...
    if (ip_header->ip_p == IPPROTO_UDP) {
        ...
        for (int i = 0; i < ntohs(dns_header_p->qdcount) && !err; i++) {
            err = process_response_question_v(
        ...
        // packet prevent this.
        log_fatal("dnsv", "Die. This can only happen if you "
...
probe_module_t module_dnsv = {
    ...
    .port_args       = 1,
    .global_init     = &dnsv_global_init,
    .thread_init     = &dnsv_thread_init,
    .make_packet     = &dnsv_make_packet,
    .print_packet    = &dnsv_print_packet,
    .validate_packet = &dnsv_validate_packet,
    .process_packet  = &dnsv_process_packet,
    .close           = &dnsv_global_cleanup,
    .output_type     = OUTPUT_TYPE_DYNAMIC,
    .fields          = fields,
    .numfields       = sizeof(fields) / sizeof(fields[0]),
    .helptext =
        "This module sends out DNS queries and parses responses for version.\n"
