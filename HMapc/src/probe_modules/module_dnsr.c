...
/* Module for stateful scanning for open UDP DNS resolvers.
 * Once a non-NS dns response is received, a non-recursive NS query will be
 * sent.
 *
 * This module optionally takes in an argument of the form
 * "LABEL_TYPE:INPUT_SRC:TYPE,QUESTION", e.g., raw:text:A,qq.com,
 * str:www:text:A,qq.com;AAAA,qq.com, random:file:file_name
 ...
 *
 * Usage: xmap -p 53 --probe-module=dnsr --probe-args="raw:text:A,qq.com"
...
// xmap boilerplate
probe_module_t module_dnsr;
static int     dns_num_ports_r;

const char     default_domain_r[] = "www.qq.com";
const uint16_t default_qtype_r    = DNS_QTYPE_A;
const char    *dnsr_usage_error   = "unknown DNS probe specification (expected "
                                    "raw/time/random:text:TYPE,QUESTION or "
                                    "raw/time/random:file:file_name or "
                                    "str:some_text:text:TYPE,QUESTION or "
                                    "str:some_text:file:file_name)";

const unsigned char *charset_alpha_lower_r =
    (unsigned char *) "abcdefghijklmnopqrstuvwxyz";

static char    **dns_packets_r;
static uint16_t *dns_packet_lens_r; // Not including udp header
static uint16_t *qname_lens_r;      // domain_len list
static char    **qnames_r;          // domain list for query
static uint16_t *qtypes_r;          // query_type list
static char    **domains_r;         // domain strs
static int       num_questions_r   = 0;
static int       index_questions_r = 0;

/* Array of qtypes_r we support. Jumping through some hops (1 level of
 * indirection) so the per-packet processing time is fast. Keep this in sync
 * with: dns_qtype (.h) qtype_strid_to_qtype_r (below) qtype_qtype_to_strid_r
...
int8_t qtype_qtype_to_strid_r[256] = {BAD_QTYPE_VAL};

void setup_qtype_str_map_r() {
    qtype_qtype_to_strid_r[DNS_QTYPE_A]     = 0;
    qtype_qtype_to_strid_r[DNS_QTYPE_NS]    = 1;
    qtype_qtype_to_strid_r[DNS_QTYPE_CNAME] = 2;
    qtype_qtype_to_strid_r[DNS_QTYPE_SOA]   = 3;
    qtype_qtype_to_strid_r[DNS_QTYPE_PTR]   = 4;
    qtype_qtype_to_strid_r[DNS_QTYPE_MX]    = 5;
    qtype_qtype_to_strid_r[DNS_QTYPE_TXT]   = 6;
    qtype_qtype_to_strid_r[DNS_QTYPE_AAAA]  = 7;
    qtype_qtype_to_strid_r[DNS_QTYPE_RRSIG] = 8;
    qtype_qtype_to_strid_r[DNS_QTYPE_ALL]   = 9;
}

static uint16_t qtype_str_to_code_r(const char *str) {
    for (int i = 0; i < qtype_strs_len_r; i++) {
        if (strcmp(qtype_strs_r[i], str) == 0) return qtype_strid_to_qtype_r[i];
    }

    return 0;
}

static char    *label_r      = NULL;
static uint16_t label_len_r  = 0;
static uint16_t label_type_r = DNS_LTYPE_RAW;
static uint16_t recursive_r  = 1;

static uint16_t domain_to_qname_r(char **qname_handle, const char *domain) {
    if (domain[0] == '.') {
...
static int build_global_dns_packets_r(char **domains, int num_domains) {
    for (int i = 0; i < num_domains; i++) {
        qname_lens_r[i] = domain_to_qname_r(&qnames_r[i], domains[i]);
        if (domains[i] != (char *) default_domain_r) {
            free(domains[i]);
        }
        dns_packet_lens_r[i] =
            sizeof(dns_header) + qname_lens_r[i] + sizeof(dns_question_tail);
        if (dns_packet_lens_r[i] > DNS_SEND_LEN) {
            log_fatal("dnsr", "DNS packet bigger (%d) than our limit (%d)",
                      dns_packet_lens_r[i], DNS_SEND_LEN);
            return EXIT_FAILURE;
        }

        dns_packets_r[i]                = xmalloc(dns_packet_lens_r[i]);
        dns_header        *dns_header_p = (dns_header *) dns_packets_r[i];
        char              *qname_p      = dns_packets_r[i] + sizeof(dns_header);
        dns_question_tail *tail_p =
            (dns_question_tail *) (dns_packets_r[i] + sizeof(dns_header) +
                                   qname_lens_r[i]);

        // All other header fields should be 0. Except id, which we set
        // per thread. Please recurse as needed.
        dns_header_p->rd = recursive_r; // Is one bit. Don't need htons
        // We have 1 question
        dns_header_p->qdcount = htons(1);
        memcpy(qname_p, qnames_r[i], qname_lens_r[i]);
        // Set the qtype to what we passed from args
        tail_p->qtype = htons(qtypes_r[i]);
...
static uint16_t get_name_helper_r(const char *data, uint16_t data_len,
    ...
    if (recursion_level > MAX_LABEL_RECURSION) {
        log_trace("dnsr", "_get_name_helper OUT. ERR, MAX RECUSION");
        return 0;
    }
    ...
    while (data_len > 0) {
        ...
        // Is this a pointer?
        if (byte >= 0xc0) {
            log_trace("dnsr", "_get_name_helper, ptr encountered");
            // Do we have enough bytes to check ahead?
            if (data_len < 2) {
                log_trace("dnsr", "_get_name_helper OUT. ptr byte encountered. "
                                  "No offset. ERR.");
                return 0;
            }
            ...
            log_trace("dnsr", "_get_name_helper. ptr offset 0x%x", offset);
            ...
            if (recursion_level > 0 || bytes_consumed > 0) {
                ...
                if (name_len < 1) {
                    log_warn("dnsr", "Exceeded static name field allocation.");
                    return 0;
                }
            ...
            uint16_t rec_bytes_consumed = get_name_helper_r(
            ...
            if (rec_bytes_consumed == 0) {
                log_trace("dnsr", "_get_name_helper OUT. rec level %d failed",
                          recursion_level);
                return 0;
            } else {
                bytes_consumed += 2;
            ...
            log_trace("dnsr", "_get_name_helper, segment 0x%hx encountered",
                      byte);
            ...
            // null byte too)?
            if ((byte + 1) > data_len) {
                log_trace("dnsr", "_get_name_helper OUT. ERR. Not enough data "
                                  "for segment %hd");
                return 0;
            }
...
static char *get_name_r(const char *data, uint16_t data_len,
                        const char *payload, uint16_t payload_len,
                        uint16_t *bytes_consumed) {
    log_trace("dnsr", "call to get_name_r, data_len: %d", data_len);
    char *name      = xmalloc(MAX_NAME_LENGTH);
    *bytes_consumed = get_name_helper_r(data, data_len, payload, payload_len,
    ...
    log_trace("dnsr",
              "return success from get_name_r, bytes_consumed: %d, string: %s",
              *bytes_consumed, name);

    return name;
}

static bool process_response_question_r(char **data, uint16_t *data_len,
                                        const char *payload,
    ...
    uint16_t bytes_consumed = 0;
    char    *question_name =
        get_name_r(*data, *data_len, payload, payload_len, &bytes_consumed);
    // Error.
    ...
    if (qtype > MAX_QTYPE || qtype_qtype_to_strid_r[qtype] == BAD_QTYPE_VAL) {
        ...
        fs_add_string(qfs, "qtype_str",
                      (char *) qtype_strs_r[qtype_qtype_to_strid_r[qtype]], 0);
    }
...
static bool process_response_answer_r(char **data, uint16_t *data_len,
                                      const char *payload, uint16_t payload_len,
                                      fieldset_t *list) {
    log_trace("dnsr", "call to process_response_answer_r, data_len: %d",
              *data_len);
    // Payload is the start of the DNS packet, including header
    // data is handle to the start of this RR
    // data_len is a pointer to the how much total data we have to work
    ...
    if (type > MAX_QTYPE || qtype_qtype_to_strid_r[type] == BAD_QTYPE_VAL) {
        fs_add_string(afs, "type_str", (char *) BAD_QTYPE_STR, 0);
    } else {
        // I've written worse things than this 3rd arg. But I want to be
        // fast.
        fs_add_string(afs, "type_str",
                      (char *) qtype_strs_r[qtype_qtype_to_strid_r[type]], 0);
    }
    ...
    if (type == DNS_QTYPE_NS || type == DNS_QTYPE_CNAME) {
        ...
        char    *rdata_name = get_name_r(rdata, rdlength, payload, payload_len,
        ...
        if (rdlength <= 4) {
            ...
            char *rdata_name = get_name_r(rdata + 2, rdlength - 2, payload,
        ...
        if (rdlength >= 1 && (rdlength - 1) != *(uint8_t *) rdata) {
            log_warn("dnsr", "TXT record with wrong TXT len. Not processing.");
            fs_add_uint64(afs, "rdata_is_parsed", 0);
            fs_add_binary(afs, "rdata", rdlength, rdata, 0);
        } else if (rdlength < 1) {
            fs_add_uint64(afs, "rdata_is_parsed", 0);
        ...
        if (rdlength != 4) {
            log_warn("dnsr", "A record with IP of length %d. Not processing.",
                     rdlength);
            fs_add_uint64(afs, "rdata_is_parsed", 0);
            fs_add_binary(afs, "rdata", rdlength, rdata, 0);
        } else {
    ...
    *data_len = *data_len - bytes_consumed - sizeof(dns_answer_tail) - rdlength;
    log_trace("dnsr",
              "return success from process_response_answer_r, data_len: %d",
              *data_len);

    return 0;
}

static int load_question_from_str_r(const char *type_q_str) {
    ...
    while (1) {
        ...
        if (probe_q_delimiter_p == type_q_str ||
            ...
            log_error("dnsr", dnsr_usage_error);
            return EXIT_FAILURE;
        }

        if (index_questions_r >= num_questions_r) {
            log_error("dnsr", "less probes than questions configured. Add "
        ...
        if (label_type_r == DNS_LTYPE_STR) {
            domains_r[index_questions_r] =
                xmalloc(label_len_r + 1 + domain_len + 1);
            strncpy(domains_r[index_questions_r], label_r, label_len_r);
            domains_r[index_questions_r][label_len_r] = '.';
            strncpy(domains_r[index_questions_r] + label_len_r + 1,
                    probe_q_delimiter_p + 1, domain_len);
            domains_r[index_questions_r][label_len_r + 1 + domain_len] = '\0';
        } else {
            domains_r[index_questions_r] = xmalloc(domain_len + 1);
            strncpy(domains_r[index_questions_r], probe_q_delimiter_p + 1,
                    domain_len);
            domains_r[index_questions_r][domain_len] = '\0';
        }
        ...
        qtypes_r[index_questions_r] = qtype_str_to_code_r(strupr(qtype_str));
        if (!qtypes_r[index_questions_r]) {
            log_error("dnsr", "incorrect qtype supplied. %s", qtype_str);
            free(qtype_str);
            return EXIT_FAILURE;
        }
        free(qtype_str);
...
static int load_question_from_file_r(const char *file) {
    log_debug("dnsr", "load dns query domains from file");

    FILE *fp = fopen(file, "r");
    if (fp == NULL) {
        log_error("dnsr", "null dns domain file");
        return EXIT_FAILURE;
    }
    ...
    while (!feof(fp)) {
        ...
        pos = strchr(line, '\n');
        if (pos != NULL) *pos = '\0';
        if (load_question_from_str_r(line)) return EXIT_FAILURE;
    }

    return EXIT_SUCCESS;
}

int dns_random_bytes_r(char *dst, int len, const unsigned char *charset,
...
static int dnsr_global_init(struct state_conf *conf) {
    num_questions_r = conf->target_index_num;
    ...
    if (num_questions_r < 1) {
        log_fatal("dnsr", "invalid number of probes for the DNS module: %d",
                  num_questions_r);
    }

    // Setup the global structures
    dns_packets_r     = xmalloc(sizeof(char *) * num_questions_r);
    dns_packet_lens_r = xmalloc(sizeof(uint16_t) * num_questions_r);
    qname_lens_r      = xmalloc(sizeof(uint16_t) * num_questions_r);
    qnames_r          = xmalloc(sizeof(char *) * num_questions_r);
    qtypes_r          = xmalloc(sizeof(uint16_t) * num_questions_r);
    domains_r         = xmalloc(sizeof(char *) * num_questions_r);

    for (int i = 0; i < num_questions_r; i++) {
        domains_r[i] = (char *) default_domain_r;
        qtypes_r[i]  = default_qtype_r;
    }

    // This is xmap boilerplate. Why do I have to write this?
    dns_num_ports_r = conf->source_port_last - conf->source_port_first + 1;
    setup_qtype_str_map_r();
    ...
    if (conf->probe_args &&
        ...
        if (strncasecmp(conf->probe_args, "raw", 3) == 0) {
            label_type_r = DNS_LTYPE_RAW;
            log_debug("dnsr", "raw label prefix");
        } else if (strncasecmp(conf->probe_args, "time", 4) == 0) {
            label_type_r = DNS_LTYPE_TIME;
            log_debug("dnsr", "time label prefix");
        } else if (strncasecmp(conf->probe_args, "random", 6) == 0) {
            label_type_r = DNS_LTYPE_RANDOM;
            log_debug("dnsr", "random label prefix");
        } else if (strncasecmp(conf->probe_args, "str", 3) == 0) {
            label_type_r     = DNS_LTYPE_STR;
            conf->probe_args = c;
            ...
            label_len_r = c - conf->probe_args;
            label_r     = xmalloc(label_len_r);
            strncpy(label_r, conf->probe_args, label_len_r);
            ++c;
            log_debug("dnsr", "label prefix: %s, len: %d", label_r,
                      label_len_r);
        } else {
            log_error("dnsr", dnsr_usage_error);
            return EXIT_FAILURE;
        ...
        if (strncasecmp(conf->probe_args, "text", 4) == 0) {
            if (load_question_from_str_r(c)) return EXIT_FAILURE;
        } else if (strncasecmp(conf->probe_args, "file", 4) == 0) {
            if (load_question_from_file_r(c)) return EXIT_FAILURE;
        } else {
            log_error("dnsr", dnsr_usage_error);
            return EXIT_FAILURE;
        }
        ...
        if (index_questions_r < num_questions_r) {
            log_error("dnsr", "more probes than questions configured. Add "
                              "additional probes.");
            return EXIT_FAILURE;
        }
    }

    if (label_type_r == DNS_LTYPE_RAW || label_type_r == DNS_LTYPE_STR)
        return build_global_dns_packets_r(domains_r, num_questions_r);
    else
        return EXIT_SUCCESS;
}

static int dnsr_global_cleanup(UNUSED struct state_conf *xconf,
                               UNUSED struct state_send *xsend,
                               UNUSED struct state_recv *xrecv) {
    if (dns_packets_r) {
        for (int i = 0; i < num_questions_r; i++) {
            if (dns_packets_r[i]) {
                free(dns_packets_r[i]);
            }
        }
        free(dns_packets_r);
    }
    ...
    if (dns_packet_lens_r) {
        free(dns_packet_lens_r);
    }
...
int dnsr_thread_init(void *buf, macaddr_t *src, macaddr_t *gw, void **arg_ptr) {
    memset(buf, 0, MAX_PACKET_SIZE);

    // Setup assuming num_questions_r == 0
    struct ether_header *eth_header = (struct ether_header *) buf;
    make_eth_header(eth_header, src, gw);

    struct ip *ip_header = (struct ip *) (&eth_header[1]);
    uint16_t   ip_len =
        sizeof(struct ip) + sizeof(struct udphdr) + dns_packet_lens_r[0];
    make_ip_header(ip_header, IPPROTO_UDP, ip_len);

    struct udphdr *udp_header = (struct udphdr *) (&ip_header[1]);
    uint16_t       udp_len    = sizeof(struct udphdr) + dns_packet_lens_r[0];
    make_udp_header(udp_header, udp_len);

    char *payload             = (char *) (&udp_header[1]);
    module_dnsr.packet_length = sizeof(struct ether_header) +
                                sizeof(struct ip) + sizeof(struct udphdr) +
                                dns_packet_lens_r[0];
    assert(module_dnsr.packet_length <= MAX_PACKET_SIZE);

    memcpy(payload, dns_packets_r[0], dns_packet_lens_r[0]);
...
int dnsr_make_packet(void *buf, size_t *buf_len, ipaddr_n_t *src_ip,
                     ipaddr_n_t *dst_ip, port_h_t dst_port, uint8_t ttl,
                     UNUSED int probe_num, index_h_t index, void *arg) {
    struct ether_header *eth_header = (struct ether_header *) buf;
    struct ip           *ip_header  = (struct ip *) (&eth_header[1]);
    struct udphdr       *udp_header = (struct udphdr *) (&ip_header[1]);

    if (label_type_r == DNS_LTYPE_RAW || label_type_r == DNS_LTYPE_STR) {
        // For num_questions_r == 1, we handle this in per-thread init. Do less
        // work
        if (num_questions_r > 1) {
            uint16_t ip_len = sizeof(struct ip) + sizeof(struct udphdr) +
                              dns_packet_lens_r[index];
            make_ip_header(ip_header, IPPROTO_UDP, ip_len);

            uint16_t udp_len = sizeof(struct udphdr) + dns_packet_lens_r[index];
            make_udp_header(udp_header, udp_len);

            char *payload = (char *) (&udp_header[1]);
            *buf_len      = sizeof(struct ether_header) + sizeof(struct ip) +
                       sizeof(struct udphdr) + dns_packet_lens_r[index];

            assert(*buf_len <= MAX_PACKET_SIZE);

            memcpy(payload, dns_packets_r[index], dns_packet_lens_r[index]);
        }
        ...
        udp_header->uh_sport =
            htons(get_src_port(dns_num_ports_r, probe_num, validation));
        ...
        int   new_label_len = 18;
        char *new_label     = xmalloc(new_label_len);

        switch (label_type_r) {
        case DNS_LTYPE_TIME: {
            struct timeval t;
            gettimeofday(&t, NULL);
            snprintf(new_label, new_label_len, "%u-%06u", (uint64_t) t.tv_sec,
                     (uint64_t) t.tv_usec);
            new_label[new_label_len - 1] = '\0';
            break;
        }
        case DNS_LTYPE_RANDOM: {
            aesrand_t *aes = (aesrand_t *) arg;
            dns_random_bytes_r(new_label, new_label_len - 1,
                               charset_alpha_lower_r, 26, aes);
            new_label[new_label_len - 1] = '\0';
            break;
        }
        default:
            log_fatal("dnsr", dnsr_usage_error);
            return EXIT_FAILURE;
        }
        ...
        if (dns_packet_lens_r[index] > DNS_SEND_LEN) {
            log_fatal("dnsr", "DNS packet bigger (%d) than our limit (%d)",
                      dns_packet_lens_r[index], DNS_SEND_LEN);
            return EXIT_FAILURE;
        }
        ...
        char              *qname_p = dns_packets_r[index] + sizeof(dns_header);
        dns_question_tail *tail_p =
            (dns_question_tail *) (dns_packets_r[index] + sizeof(dns_header) +
                                   qname_lens_r[index]);
        ...
        // We have 1 question
        dns_header_p->qdcount = htons(1);
        memcpy(qname_p, qnames_r[index], qname_lens_r[index]);
        // Set the qtype to what we passed from args
        tail_p->qtype = htons(qtypes_r[index]);
        // Set the qclass to The Internet (TM) (R) (I hope you're happy
        // now Zakir)
        tail_p->qclass = htons(0x01);
        // MAGIC NUMBER. Let's be honest. This is only ever 1
        ...
        *buf_len      = sizeof(struct ether_header) + sizeof(struct ip) +
        ...
        udp_header->uh_sport =
            htons(get_src_port(dns_num_ports_r, index, validation));
        udp_header->uh_dport = htons(dst_port);
...
void dnsr_print_packet(FILE *fp, void *packet) {
    ...
    fprintf(fp,
            ...
            question_name,
            qtype_strs_r[qtype_qtype_to_strid_r[ntohs(tail_p->qtype)]]);
    fprintf(fp, "------------------------------------------------------\n");

    free(question_name);
}

int dnsr_validate_packet(const struct ip *ip_hdr, uint32_t len,
    ...
    if (ip_hdr->ip_p == IPPROTO_UDP) {
        ...
        if (!check_dns_src_port(sport, dns_num_ports_r, validation)) {
            return PACKET_INVALID;
        }
        ...
        uint16_t dns_type = tail_p->qtype;

        // NS query
        if (ntohs(tail_p->qtype) == DNS_QTYPE_NS) {
            ...
            ip_type_domain[xconf.ipv46_bytes + 1] =
                (char) (tail_p->qtype & 0xffu);
            memcpy(ip_type_domain + xconf.ipv46_bytes + 2, qname,
                   strlen(qname));
            if (bloom_filter_check_string(
                    &xrecv.bf, (const char *) ip_type_domain,
                    ip_type_domain_len) == BLOOM_FAILURE) {
                bloom_filter_add_string(&xrecv.bf,
                                        (const char *) ip_type_domain,
                                        ip_type_domain_len);
                *is_repeat = 0;
            }
            free(ip_type_domain);
        ...
        //        if (dns_header_p->rcode != DNS_RCODE_NOERR &&
        //        dns_header_p->rcode != DNS_RCODE_NXDOMAIN)
        //            return PACKET_INVALID;

        //        if (dns_header_p->rcode != DNS_RCODE_NXDOMAIN)
        //            return PACKET_INVALID;

        if (dns_header_p->rcode != DNS_RCODE_NOERR) return PACKET_INVALID;

        //        // ignore first label
        //        qname = qname + (int) qname[0] + 1;
        //        qname_len = strlen(qname) + 1;

        // non NS query, new query
        // dns qname
        int dns_packet_len =
            sizeof(dns_header) + qname_len + sizeof(dns_question_tail);
        char *dns_packet = xmalloc(dns_packet_len);
        dns_header_p     = (dns_header *) dns_packet;
        char *qname_p    = dns_packet + sizeof(dns_header);
        tail_p =
            (dns_question_tail *) (dns_packet + sizeof(dns_header) + qname_len);

        // All other header fields should be 0. Except id, which we set
        // per thread. Please recurse as needed.
        dns_header_p->rd = 0; // Is one bit. Don't need htons, non-recursive
        // We have 1 question
        dns_header_p->qdcount = htons(1);
        memcpy(qname_p, qname, qname_len);
        // Set the qtype to NS
        tail_p->qtype = htons(DNS_QTYPE_NS);
        // Set the qclass to The Internet (TM) (R) (I hope you're happy
        // now Zakir)
        tail_p->qclass = htons(0x01);
        // MAGIC NUMBER. Let's be honest. This is only ever 1
        ...
        uint16_t ip_len =
            sizeof(struct ip) + sizeof(struct udphdr) + dns_packet_len;
        make_ip_header(ip_header, IPPROTO_UDP, ip_len);

        uint16_t udp_len = sizeof(struct udphdr) + dns_packet_len;
        make_udp_header(udp_header, udp_len);
        ...
        *buf_len      = sizeof(struct ether_header) + sizeof(struct ip) +
        ...
        memcpy(ip_type_domain, (char *) &(ip_hdr->ip_src.s_addr),
               xconf.ipv46_bytes);
        ip_type_domain[xconf.ipv46_bytes]     = (char) (dns_type >> 8u);
        ip_type_domain[xconf.ipv46_bytes + 1] = (char) (dns_type & 0xffu);
...
void dnsr_process_packet(const u_char *packet, uint32_t len, fieldset_t *fs,
    ...
    if (ip_header->ip_p == IPPROTO_UDP) {
        ...
        for (int i = 0; i < ntohs(dns_header_p->qdcount) && !err; i++) {
            err = process_response_question_r(
        ...
        // packet prevent this.
        log_fatal("dnsr", "Die. This can only happen if you "
...
probe_module_t module_dnsr = {
    ...
    .port_args       = 1,
    .global_init     = &dnsr_global_init,
    .thread_init     = &dnsr_thread_init,
    .make_packet     = &dnsr_make_packet,
    .print_packet    = &dnsr_print_packet,
    .validate_packet = &dnsr_validate_packet,
    .process_packet  = &dnsr_process_packet,
    .close           = &dnsr_global_cleanup,
    .output_type     = OUTPUT_TYPE_DYNAMIC,
    .fields          = fields,
    .numfields       = sizeof(fields) / sizeof(fields[0]),
    .helptext =
        "This module sends out DNS queries and parses basic responses.\n"
        "Once a non-NS reply is received, a non-recurse NS query will be "
        "sent.\n"
