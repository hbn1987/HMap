 ...
 *
 * Usage: xmap -p 53 --probe-module=dnsae --probe-args="raw:text:A,qq.com"
...
// xmap boilerplate
probe_module_t module_dnsae;
static int     dns_num_ports_ae;

const char     default_domain_ae[] = "www.qq.com";
const uint16_t default_qtype_ae    = DNS_QTYPE_A;
const char    *dnsae_usage_error =
...
const unsigned char *charset_alpha_lower_ae =
    (unsigned char *) "abcdefghijklmnopqrstuvwxyz";

static char    **dns_packets_ae;
static uint16_t *dns_packet_lens_ae; // Not including udp header
static uint16_t *qname_lens_ae;      // domain_len list
static char    **qnames_ae;          // domain list for query
static uint16_t *qtypes_ae;          // query_type list
static char    **domains_ae;         // domain strs
static int       num_questions_ae   = 0;
static int       index_questions_ae = 0;

const char      default_option_qname_ae[]   = {0x00};
static int      default_option_qname_len_ae = 1;
static uint16_t default_option_udpsize_ae   = 4096;
const char      default_option_rdata_ae[];
static int      default_option_rdata_len_ae = 0;

/* Array of qtypes_ae we support. Jumping through some hops (1 level of
 * indirection) so the per-packet processing time is fast. Keep this in sync
 * with: dns_qtype (.h) qtype_strid_to_qtype_ae (below)
...
int8_t qtype_qtype_to_strid_ae[65536] = {BAD_QTYPE_VAL};

void setup_qtype_str_map_ae() {
    qtype_qtype_to_strid_ae[DNS_QTYPE_A]        = 0;
    qtype_qtype_to_strid_ae[DNS_QTYPE_NS]       = 1;
    qtype_qtype_to_strid_ae[DNS_QTYPE_CNAME]    = 2;
    qtype_qtype_to_strid_ae[DNS_QTYPE_SOA]      = 3;
    qtype_qtype_to_strid_ae[DNS_QTYPE_PTR]      = 4;
    qtype_qtype_to_strid_ae[DNS_QTYPE_MX]       = 5;
    qtype_qtype_to_strid_ae[DNS_QTYPE_TXT]      = 6;
    qtype_qtype_to_strid_ae[DNS_QTYPE_AAAA]     = 7;
    qtype_qtype_to_strid_ae[DNS_QTYPE_RRSIG]    = 8;
    qtype_qtype_to_strid_ae[DNS_QTYPE_ALL]      = 9;
    qtype_qtype_to_strid_ae[DNS_QTYPE_SIG]      = 10;
    qtype_qtype_to_strid_ae[DNS_QTYPE_SRV]      = 11;
    qtype_qtype_to_strid_ae[DNS_QTYPE_DS]       = 12;
    qtype_qtype_to_strid_ae[DNS_QTYPE_DNSKEY]   = 13;
    qtype_qtype_to_strid_ae[DNS_QTYPE_TLSA]     = 14;
    qtype_qtype_to_strid_ae[DNS_QTYPE_SVCB]     = 15;
    qtype_qtype_to_strid_ae[DNS_QTYPE_HTTPS]    = 16;
    qtype_qtype_to_strid_ae[DNS_QTYPE_CAA]      = 17;
    qtype_qtype_to_strid_ae[DNS_QTYPE_HTTPSSVC] = 18;
    qtype_qtype_to_strid_ae[DNS_QTYPE_OPT]      = 19;
}

static uint16_t qtype_str_to_code_ae(const char *str) {
    for (int i = 0; i < qtype_strs_len_ae; i++) {
        if (strcmp(qtype_strs_ae[i], str) == 0)
            return qtype_strid_to_qtype_ae[i];
    }

    return 0;
}

static char    *label_ae      = NULL;
static uint16_t label_len_ae  = 0;
static uint16_t label_type_ae = DNS_LTYPE_RAW;
static uint16_t recursive_ae  = 1;

static uint16_t domain_to_qname_ae(char **qname_handle, const char *domain) {
    if (domain[0] == '.') {
...
static int build_global_dns_packets_ae(char **domains, int num_domains) {
    for (int i = 0; i < num_domains; i++) {
        qname_lens_ae[i] = domain_to_qname_ae(&qnames_ae[i], domains[i]);
        if (domains[i] != (char *) default_domain_ae) {
            free(domains[i]);
        }
        dns_packet_lens_ae[i] =
            sizeof(dns_header) + qname_lens_ae[i] + sizeof(dns_question_tail) +
            default_option_qname_len_ae + sizeof(dns_option_tail) +
            default_option_rdata_len_ae;
        if (dns_packet_lens_ae[i] > DNS_SEND_LEN) {
            log_fatal("dnsae", "DNS packet bigger (%d) than our limit (%d)",
                      dns_packet_lens_ae[i], DNS_SEND_LEN);
            return EXIT_FAILURE;
        }
        ...
        dns_question_tail *tail_p =
            (dns_question_tail *) (dns_packets_ae[i] + sizeof(dns_header) +
                                   qname_lens_ae[i]);
        char *option_qname_p =
            (char *) (dns_packets_ae[i] + sizeof(dns_header) +
                      qname_lens_ae[i] + sizeof(dns_question_tail));
        dns_option_tail *option_tail_p =
            (dns_option_tail *) (dns_packets_ae[i] + sizeof(dns_header) +
                                 qname_lens_ae[i] + sizeof(dns_question_tail) +
                                 default_option_qname_len_ae);

        // All other header fields should be 0. Except id, which we set
        // per thread. Please recurse as needed.
        dns_header_p->rd = recursive_ae; // Is one bit. Don't need htons
        // We have 1 question
        dns_header_p->qdcount = htons(1);
        memcpy(qname_p, qnames_ae[i], qname_lens_ae[i]);
        // Set the qtype to what we passed from args
        tail_p->qtype = htons(qtypes_ae[i]);
        ...
        memcpy(option_qname_p, default_option_qname_ae,
               default_option_qname_len_ae);
        option_tail_p->type    = htons(DNS_QTYPE_OPT);
        option_tail_p->udpsize = htons(default_option_udpsize_ae);
    }
...
static uint16_t get_name_helper_ae(const char *data, uint16_t data_len,
    ...
    if (recursion_level > MAX_LABEL_RECURSION) {
        log_trace("dnsae", "_get_name_helper OUT. ERR, MAX RECUSION");
        return 0;
    }
    ...
    while (data_len > 0) {
        ...
        // Is this a pointer?
        if (byte >= 0xc0) {
            log_trace("dnsae", "_get_name_helper, ptr encountered");
            ...
            log_trace("dnsae", "_get_name_helper. ptr offset 0x%x", offset);
            ...
            uint16_t rec_bytes_consumed = get_name_helper_ae(
            ...
            if (rec_bytes_consumed == 0) {
                log_trace("dnsae", "_get_name_helper OUT. rec level %d failed",
                          recursion_level);
                return 0;
            } else {
                bytes_consumed += 2;
            ...
            log_trace("dnsae", "_get_name_helper, segment 0x%hx encountered",
                      byte);
...
static bool process_response_question_ae(char **data, uint16_t *data_len,
    ...
    uint16_t bytes_consumed = 0;
    char    *question_name =
        get_name_ae(*data, *data_len, payload, payload_len, &bytes_consumed);
    // Error.
    ...
    if (qtype > MAX_QTYPE || qtype_qtype_to_strid_ae[qtype] == BAD_QTYPE_VAL) {
        ...
        fs_add_string(qfs, "qtype_str",
                      (char *) qtype_strs_ae[qtype_qtype_to_strid_ae[qtype]],
                      0);
    }
...
static bool process_response_answer_ae(char **data, uint16_t *data_len,
    ...
    if (type > MAX_QTYPE || qtype_qtype_to_strid_ae[type] == BAD_QTYPE_VAL) {
    ...
    if (type == DNS_QTYPE_NS || type == DNS_QTYPE_CNAME) {
        ...
        if (rdlength != 4) {
            log_warn("dnsae", "A record with IP of length %d. Not processing.",
                     rdlength);
            fs_add_uint64(afs, "rdata_is_parsed", 0);
            fs_add_binary(afs, "rdata", rdlength, rdata, 0);
        } else {
    ...
    *data_len = *data_len - bytes_consumed - sizeof(dns_answer_tail) - rdlength;
    log_trace("dnsae",
              "return success from process_response_answer_ae, data_len: %d",
              *data_len);

    return 0;
}

static int load_question_from_str_ae(const char *type_q_str) {
    ...
    while (1) {
        ...
        if (probe_q_delimiter_p == type_q_str ||
            ...
            log_error("dnsae", dnsae_usage_error);
            return EXIT_FAILURE;
        }
        ...
        if (label_type_ae == DNS_LTYPE_STR) {
            domains_ae[index_questions_ae] =
                xmalloc(label_len_ae + 1 + domain_len + 1);
            strncpy(domains_ae[index_questions_ae], label_ae, label_len_ae);
            domains_ae[index_questions_ae][label_len_ae] = '.';
            strncpy(domains_ae[index_questions_ae] + label_len_ae + 1,
                    probe_q_delimiter_p + 1, domain_len);
            domains_ae[index_questions_ae][label_len_ae + 1 + domain_len] =
                '\0';
        } else {
            domains_ae[index_questions_ae] = xmalloc(domain_len + 1);
            strncpy(domains_ae[index_questions_ae], probe_q_delimiter_p + 1,
                    domain_len);
            domains_ae[index_questions_ae][domain_len] = '\0';
        }
...
static int load_question_from_file_ae(const char *file) {
    log_debug("dnsae", "load dns query domains from file");

    FILE *fp = fopen(file, "r");
    if (fp == NULL) {
        log_error("dnsae", "null dns domain file");
        return EXIT_FAILURE;
    }
    ...
    while (!feof(fp)) {
        ...
        pos = strchr(line, '\n');
        if (pos != NULL) *pos = '\0';
        if (load_question_from_str_ae(line)) return EXIT_FAILURE;
    }
...
static int dnsae_global_init(struct state_conf *conf) {
    num_questions_ae = conf->target_index_num;
    ...
    if (num_questions_ae < 1) {
        log_fatal("dnsae", "invalid number of probes for the DNS module: %d",
                  num_questions_ae);
    }

    // Setup the global structures
    dns_packets_ae     = xmalloc(sizeof(char *) * num_questions_ae);
    dns_packet_lens_ae = xmalloc(sizeof(uint16_t) * num_questions_ae);
    qname_lens_ae      = xmalloc(sizeof(uint16_t) * num_questions_ae);
    qnames_ae          = xmalloc(sizeof(char *) * num_questions_ae);
    qtypes_ae          = xmalloc(sizeof(uint16_t) * num_questions_ae);
    domains_ae         = xmalloc(sizeof(char *) * num_questions_ae);

    for (int i = 0; i < num_questions_ae; i++) {
        domains_ae[i] = (char *) default_domain_ae;
        qtypes_ae[i]  = default_qtype_ae;
    }

    // This is xmap boilerplate. Why do I have to write this?
    dns_num_ports_ae = conf->source_port_last - conf->source_port_first + 1;
    setup_qtype_str_map_ae();
    ...
    if (conf->probe_args &&
        ...
        if (strncasecmp(conf->probe_args, "raw", 3) == 0) {
            label_type_ae = DNS_LTYPE_RAW;
            log_debug("dnsae", "raw label prefix");
        } else if (strncasecmp(conf->probe_args, "time", 4) == 0) {
            label_type_ae = DNS_LTYPE_TIME;
            log_debug("dnsae", "time label prefix");
            ...
            label_len_ae = c - conf->probe_args;
            label_ae     = xmalloc(label_len_ae);
            strncpy(label_ae, conf->probe_args, label_len_ae);
            ++c;
            log_debug("dnsae", "label prefix: %s, len: %d", label_ae,
                      label_len_ae);
        } else if (strncasecmp(conf->probe_args, "dst-ip", 6) == 0) {
            label_type_ae = DNS_LTYPE_SRCIP;
            log_debug("dnsae", "dst-ip label prefix");
        } else {
            log_error("dnsae", dnsae_usage_error);
            return EXIT_FAILURE;
        }
        ...
        if (strncasecmp(conf->probe_args, "text", 4) == 0) {
            if (load_question_from_str_ae(c)) return EXIT_FAILURE;
        } else if (strncasecmp(conf->probe_args, "file", 4) == 0) {
            if (load_question_from_file_ae(c)) return EXIT_FAILURE;
        } else {
            log_error("dnsae", dnsae_usage_error);
            return EXIT_FAILURE;
        }
    ...
    if (label_type_ae == DNS_LTYPE_RAW || label_type_ae == DNS_LTYPE_STR)
        return build_global_dns_packets_ae(domains_ae, num_questions_ae);
    else
        return EXIT_SUCCESS;
}
...
static int dnsae_global_cleanup(UNUSED struct state_conf *xconf,
    ...
    if (dns_packet_lens_ae) {
        free(dns_packet_lens_ae);
    }
...
int dnsae_thread_init(void *buf, macaddr_t *src, macaddr_t *gw,
    ...
    uint16_t   ip_len =
        sizeof(struct ip) + sizeof(struct udphdr) + dns_packet_lens_ae[0];
...
int dnsae_make_packet(void *buf, size_t *buf_len, ipaddr_n_t *src_ip,
    ...
    port_h_t src_port = get_src_port(dns_num_ports_ae, probe_num, validation);
    uint16_t dns_txid = get_dnsa_txid(validation, probe_num);

    if (label_type_ae == DNS_LTYPE_RAW || label_type_ae == DNS_LTYPE_STR) {
        // For num_questions_ae == 1, we handle this in per-thread init. Do
        // less work
        if (num_questions_ae > 1) {
            uint16_t ip_len = sizeof(struct ip) + sizeof(struct udphdr) +
                              dns_packet_lens_ae[index];
            make_ip_header(ip_header, IPPROTO_UDP, ip_len);

            uint16_t udp_len =
                sizeof(struct udphdr) + dns_packet_lens_ae[index];
            make_udp_header(udp_header, udp_len);
            ...
            *buf_len      = sizeof(struct ether_header) + sizeof(struct ip) +
            ...
            memcpy(payload, dns_packets_ae[index], dns_packet_lens_ae[index]);
        }
        ...
        default:
            log_fatal("dnsae", dnsae_usage_error);
            return EXIT_FAILURE;
        }
        ...
        // dns packet
        free(qnames_ae[index]);

        qname_lens_ae[index] =
            domain_to_qname_ae(&qnames_ae[index], new_domain);
        dns_packet_lens_ae[index] =
            sizeof(dns_header) + qname_lens_ae[index] +
            sizeof(dns_question_tail) + default_option_qname_len_ae +
            sizeof(dns_option_tail) + default_option_rdata_len_ae;
        if (dns_packet_lens_ae[index] > DNS_SEND_LEN) {
            log_fatal("dnsae", "DNS packet bigger (%d) than our limit (%d)",
                      dns_packet_lens_ae[index], DNS_SEND_LEN);
            return EXIT_FAILURE;
        }
        ...
        dns_question_tail *tail_p =
            (dns_question_tail *) (dns_packets_ae[index] + sizeof(dns_header) +
                                   qname_lens_ae[index]);
        char *option_qname_p =
            (char *) (dns_packets_ae[index] + sizeof(dns_header) +
                      qname_lens_ae[index] + sizeof(dns_question_tail));
        dns_option_tail *option_tail_p =
            (dns_option_tail *) (dns_packets_ae[index] + sizeof(dns_header) +
                                 qname_lens_ae[index] +
        ...
        // We have 1 question
        dns_header_p->qdcount = htons(1);
        memcpy(qname_p, qnames_ae[index], qname_lens_ae[index]);
        // Set the qtype to what we passed from args
        tail_p->qtype = htons(qtypes_ae[index]);
        // Set the qclass to The Internet (TM) (R) (I hope you're happy
        // now Zakir)
        tail_p->qclass = htons(0x01);
        // MAGIC NUMBER. Let's be honest. This is only ever 1
        ...
        uint16_t udp_len = sizeof(struct udphdr) + dns_packet_lens_ae[index];
        make_udp_header(udp_header, udp_len);
...
void dnsae_print_packet(FILE *fp, void *packet) {
    ...
    dns_option_tail *option_tail_p =
        (dns_option_tail *) (option_qname_p + default_option_qname_len_ae);
    ...
    fprintf(fp,
            ...
            question_name,
            qtype_strs_ae[qtype_qtype_to_strid_ae[tail_p->qtype]],
...
int dnsae_validate_packet(const struct ip *ip_hdr, uint32_t len,
    ...
    if (ip_hdr->ip_p == IPPROTO_UDP) {
        ...
        if (!check_dns_src_port(sport, dns_num_ports_ae, validation)) {
            return PACKET_INVALID;
        }
...
void dnsae_process_packet(const u_char *packet, uint32_t len, fieldset_t *fs,
    ...
    if (ip_header->ip_p == IPPROTO_UDP) {
        ...
        for (int i = 0; i < ntohs(dns_header_p->qdcount) && !err; i++) {
            err = process_response_question_ae(
        ...
        for (int i = 0; i < ntohs(dns_header_p->ancount) && !err; i++) {
            err = process_response_answer_ae(
                &data, &data_len, (char *) dns_header_p, udp_len, list);
        }
...
probe_module_t module_dnsae = {
    ...
    .port_args       = 1,
    .global_init     = &dnsae_global_init,
    .thread_init     = &dnsae_thread_init,
    .make_packet     = &dnsae_make_packet,
    .print_packet    = &dnsae_print_packet,
    .validate_packet = &dnsae_validate_packet,
    .process_packet  = &dnsae_process_packet,
    .close           = &dnsae_global_cleanup,
