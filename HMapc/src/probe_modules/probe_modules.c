...
probe_module_t *get_probe_module_by_name(const char *name, int ipv46_flag) {
    ...
    for (int i = 0; i < len; i++) {
        if (probe_modules[i]->ipv46_flag != ipv46_flag) continue;
...
void fs_add_ipv6_fields(fieldset_t *fs, struct ip6_hdr *ipv6_hdr) {
    // WARNING: you must update fs_ip_fields_len  as well
    // as the definitions set (ip6_fiels) if you
    // change the fields added below:
    fs_add_string(fs, "saddr",
                  make_ipv6_str((struct in6_addr *) &(ipv6_hdr->ip6_src)), 1);
    fs_add_string(fs, "daddr",
                  make_ipv6_str((struct in6_addr *) &(ipv6_hdr->ip6_dst)), 1);
    fs_add_uint64(fs, "hlim", ipv6_hdr->ip6_ctlun.ip6_un1.ip6_un1_hlim);
}
