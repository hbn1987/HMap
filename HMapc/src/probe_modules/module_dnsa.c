 ...
 *
 * Usage: xmap -p 53 --probe-module=dnsa --probe-args="raw:text:A,qq.com"
...
// xmap boilerplate
probe_module_t module_dnsa;
static int     dns_num_ports_a;

const char     default_domain_a[] = "www.qq.com";
const uint16_t default_qtype_a    = DNS_QTYPE_A;
const char    *dnsa_usage_error =
...
const unsigned char *charset_alpha_lower_a =
    (unsigned char *) "abcdefghijklmnopqrstuvwxyz";

static char    **dns_packets_a;
static uint16_t *dns_packet_lens_a; // Not including udp header
static uint16_t *qname_lens_a;      // domain_len list
static char    **qnames_a;          // domain list for query
static uint16_t *qtypes_a;          // query_type list
static char    **domains_a;         // domain strs
static int       num_questions_a   = 0;
static int       index_questions_a = 0;

/* Array of qtypes_a we support. Jumping through some hops (1 level of
 * indirection) so the per-packet processing time is fast. Keep this in sync
 * with: dns_qtype (.h) qtype_strid_to_qtype_a (below) qtype_qtype_to_strid_a
 * (below, and setup_qtype_str_map_a())
 */
const char *qtype_strs_a[]   = {"A",    "NS",    "CNAME", "SOA",     "PTR",
                                "MX",   "TXT",   "AAAA",  "RRSIG",   "ANY",
                                "SIG",  "SRV",   "DS",    "DNSKEY",  "TLSA",
                                "SVCB", "HTTPS", "CAA",   "HTTPSSVC"};
const int   qtype_strs_len_a = 19;

const dns_qtype qtype_strid_to_qtype_a[] = {
...
int8_t qtype_qtype_to_strid_a[65536] = {BAD_QTYPE_VAL};

void setup_qtype_str_map_a() {
    qtype_qtype_to_strid_a[DNS_QTYPE_A]        = 0;
    qtype_qtype_to_strid_a[DNS_QTYPE_NS]       = 1;
    qtype_qtype_to_strid_a[DNS_QTYPE_CNAME]    = 2;
    qtype_qtype_to_strid_a[DNS_QTYPE_SOA]      = 3;
    qtype_qtype_to_strid_a[DNS_QTYPE_PTR]      = 4;
    qtype_qtype_to_strid_a[DNS_QTYPE_MX]       = 5;
    qtype_qtype_to_strid_a[DNS_QTYPE_TXT]      = 6;
    qtype_qtype_to_strid_a[DNS_QTYPE_AAAA]     = 7;
    qtype_qtype_to_strid_a[DNS_QTYPE_RRSIG]    = 8;
    qtype_qtype_to_strid_a[DNS_QTYPE_ALL]      = 9;
    qtype_qtype_to_strid_a[DNS_QTYPE_SIG]      = 10;
    qtype_qtype_to_strid_a[DNS_QTYPE_SRV]      = 11;
    qtype_qtype_to_strid_a[DNS_QTYPE_DS]       = 12;
    qtype_qtype_to_strid_a[DNS_QTYPE_DNSKEY]   = 13;
    qtype_qtype_to_strid_a[DNS_QTYPE_TLSA]     = 14;
    qtype_qtype_to_strid_a[DNS_QTYPE_SVCB]     = 15;
    qtype_qtype_to_strid_a[DNS_QTYPE_HTTPS]    = 16;
    qtype_qtype_to_strid_a[DNS_QTYPE_CAA]      = 17;
    qtype_qtype_to_strid_a[DNS_QTYPE_HTTPSSVC] = 18;
}

static uint16_t qtype_str_to_code_a(const char *str) {
    for (int i = 0; i < qtype_strs_len_a; i++) {
        if (strcmp(qtype_strs_a[i], str) == 0) return qtype_strid_to_qtype_a[i];
    }

    return 0;
}

static char    *label_a      = NULL;
static uint16_t label_len_a  = 0;
static uint16_t label_type_a = DNS_LTYPE_RAW;
static uint16_t recursive_a  = 1;

static uint16_t domain_to_qname_a(char **qname_handle, const char *domain) {
    if (domain[0] == '.') {
...
static int build_global_dns_packets_a(char **domains, int num_domains) {
    for (int i = 0; i < num_domains; i++) {
        qname_lens_a[i] = domain_to_qname_a(&qnames_a[i], domains[i]);
        if (domains[i] != (char *) default_domain_a) {
            free(domains[i]);
        }
        dns_packet_lens_a[i] =
            sizeof(dns_header) + qname_lens_a[i] + sizeof(dns_question_tail);
        if (dns_packet_lens_a[i] > DNS_SEND_LEN) {
            log_fatal("dnsa", "DNS packet bigger (%d) than our limit (%d)",
                      dns_packet_lens_a[i], DNS_SEND_LEN);
            return EXIT_FAILURE;
        }

        dns_packets_a[i]                = xmalloc(dns_packet_lens_a[i]);
        dns_header        *dns_header_p = (dns_header *) dns_packets_a[i];
        char              *qname_p      = dns_packets_a[i] + sizeof(dns_header);
        dns_question_tail *tail_p =
            (dns_question_tail *) (dns_packets_a[i] + sizeof(dns_header) +
                                   qname_lens_a[i]);

        // All other header fields should be 0. Except id, which we set
        // per thread. Please recurse as needed.
        dns_header_p->rd = recursive_a; // Is one bit. Don't need htons
        // We have 1 question
        dns_header_p->qdcount = htons(1);
        memcpy(qname_p, qnames_a[i], qname_lens_a[i]);
        // Set the qtype to what we passed from args
        tail_p->qtype = htons(qtypes_a[i]);
...
static uint16_t get_name_helper_a(const char *data, uint16_t data_len,
    ...
    if (recursion_level > MAX_LABEL_RECURSION) {
        log_trace("dnsa", "_get_name_helper OUT. ERR, MAX RECUSION");
        return 0;
    }
    ...
    while (data_len > 0) {
        ...
        // Is this a pointer?
        if (byte >= 0xc0) {
            log_trace("dnsa", "_get_name_helper, ptr encountered");
            // Do we have enough bytes to check ahead?
            if (data_len < 2) {
                log_trace("dnsa", "_get_name_helper OUT. ptr byte encountered. "
                                  "No offset. ERR.");
                return 0;
            }
            ...
            log_trace("dnsa", "_get_name_helper. ptr offset 0x%x", offset);
            ...
            if (recursion_level > 0 || bytes_consumed > 0) {
                ...
                if (name_len < 1) {
                    log_warn("dnsa", "Exceeded static name field allocation.");
                    return 0;
                }
            ...
            uint16_t rec_bytes_consumed = get_name_helper_a(
            ...
            if (rec_bytes_consumed == 0) {
                log_trace("dnsa", "_get_name_helper OUT. rec level %d failed",
                          recursion_level);
                return 0;
            } else {
                bytes_consumed += 2;
            ...
            log_trace("dnsa", "_get_name_helper, segment 0x%hx encountered",
                      byte);
            ...
            // null byte too)?
            if ((byte + 1) > data_len) {
                log_trace("dnsa", "_get_name_helper OUT. ERR. Not enough data "
                                  "for segment %hd");
                return 0;
            }
...
static char *get_name_a(const char *data, uint16_t data_len,
                        const char *payload, uint16_t payload_len,
                        uint16_t *bytes_consumed) {
    log_trace("dnsa", "call to get_name_a, data_len: %d", data_len);
    char *name      = xmalloc(MAX_NAME_LENGTH);
    *bytes_consumed = get_name_helper_a(data, data_len, payload, payload_len,
    ...
    log_trace("dnsa",
              "return success from get_name_a, bytes_consumed: %d, string: %s",
              *bytes_consumed, name);

    return name;
}

static bool process_response_question_a(char **data, uint16_t *data_len,
                                        const char *payload,
    ...
    uint16_t bytes_consumed = 0;
    char    *question_name =
        get_name_a(*data, *data_len, payload, payload_len, &bytes_consumed);
    // Error.
    ...
    if (qtype > MAX_QTYPE || qtype_qtype_to_strid_a[qtype] == BAD_QTYPE_VAL) {
        ...
        fs_add_string(qfs, "qtype_str",
                      (char *) qtype_strs_a[qtype_qtype_to_strid_a[qtype]], 0);
    }
...
static bool process_response_answer_a(char **data, uint16_t *data_len,
                                      const char *payload, uint16_t payload_len,
                                      fieldset_t *list) {
    log_trace("dnsa", "call to process_response_answer_a, data_len: %d",
              *data_len);
    // Payload is the start of the DNS packet, including header
    // data is handle to the start of this RR
    // data_len is a pointer to the how much total data we have to work
    ...
    if (type > MAX_QTYPE || qtype_qtype_to_strid_a[type] == BAD_QTYPE_VAL) {
        fs_add_string(afs, "type_str", (char *) BAD_QTYPE_STR, 0);
    } else {
        // I've written worse things than this 3rd arg. But I want to be
        // fast.
        fs_add_string(afs, "type_str",
                      (char *) qtype_strs_a[qtype_qtype_to_strid_a[type]], 0);
    }
    ...
    if (type == DNS_QTYPE_NS || type == DNS_QTYPE_CNAME) {
        ...
        char    *rdata_name = get_name_a(rdata, rdlength, payload, payload_len,
        ...
        if (rdlength <= 4) {
            ...
            char *rdata_name = get_name_a(rdata + 2, rdlength - 2, payload,
        ...
        if (rdlength >= 1 && (rdlength - 1) != *(uint8_t *) rdata) {
            log_warn("dnsa", "TXT record with wrong TXT len. Not processing.");
            fs_add_uint64(afs, "rdata_is_parsed", 0);
            fs_add_binary(afs, "rdata", rdlength, rdata, 0);
        } else if (rdlength < 1) {
            fs_add_uint64(afs, "rdata_is_parsed", 0);
        ...
        if (rdlength != 4) {
            log_warn("dnsa", "A record with IP of length %d. Not processing.",
                     rdlength);
            fs_add_uint64(afs, "rdata_is_parsed", 0);
            fs_add_binary(afs, "rdata", rdlength, rdata, 0);
        } else {
    ...
    *data_len = *data_len - bytes_consumed - sizeof(dns_answer_tail) - rdlength;
    log_trace("dnsa",
              "return success from process_response_answer_a, data_len: %d",
              *data_len);

    return 0;
}

static int load_question_from_str_a(const char *type_q_str) {
    ...
    while (1) {
        ...
        if (probe_q_delimiter_p == type_q_str ||
            ...
            log_error("dnsa", dnsa_usage_error);
            return EXIT_FAILURE;
        }

        if (index_questions_a >= num_questions_a) {
            log_error("dnsa", "less probes than questions configured. Add "
        ...
        if (label_type_a == DNS_LTYPE_STR) {
            domains_a[index_questions_a] =
                xmalloc(label_len_a + 1 + domain_len + 1);
            strncpy(domains_a[index_questions_a], label_a, label_len_a);
            domains_a[index_questions_a][label_len_a] = '.';
            strncpy(domains_a[index_questions_a] + label_len_a + 1,
                    probe_q_delimiter_p + 1, domain_len);
            domains_a[index_questions_a][label_len_a + 1 + domain_len] = '\0';
        } else {
            domains_a[index_questions_a] = xmalloc(domain_len + 1);
            strncpy(domains_a[index_questions_a], probe_q_delimiter_p + 1,
                    domain_len);
            domains_a[index_questions_a][domain_len] = '\0';
        }
        ...
        qtypes_a[index_questions_a] = qtype_str_to_code_a(strupr(qtype_str));
        if (!qtypes_a[index_questions_a]) {
            log_error("dnsa", "incorrect qtype supplied: %s", qtype_str);
            free(qtype_str);
            return EXIT_FAILURE;
        }
        free(qtype_str);
...
static int load_question_from_file_a(const char *file) {
    log_debug("dnsa", "load dns query domains from file");

    FILE *fp = fopen(file, "r");
    if (fp == NULL) {
        log_error("dnsa", "null dns domain file");
        return EXIT_FAILURE;
    }
    ...
    while (!feof(fp)) {
        ...
        pos = strchr(line, '\n');
        if (pos != NULL) *pos = '\0';
        if (load_question_from_str_a(line)) return EXIT_FAILURE;
    }

    return EXIT_SUCCESS;
}

int dns_random_bytes_a(char *dst, int len, const unsigned char *charset,
...
static int dnsa_global_init(struct state_conf *conf) {
    num_questions_a = conf->target_index_num;
    ...
    if (num_questions_a < 1) {
        log_fatal("dnsa", "invalid number of probes for the DNS module: %d",
                  num_questions_a);
    }

    // Setup the global structures
    dns_packets_a     = xmalloc(sizeof(char *) * num_questions_a);
    dns_packet_lens_a = xmalloc(sizeof(uint16_t) * num_questions_a);
    qname_lens_a      = xmalloc(sizeof(uint16_t) * num_questions_a);
    qnames_a          = xmalloc(sizeof(char *) * num_questions_a);
    qtypes_a          = xmalloc(sizeof(uint16_t) * num_questions_a);
    domains_a         = xmalloc(sizeof(char *) * num_questions_a);

    for (int i = 0; i < num_questions_a; i++) {
        domains_a[i] = (char *) default_domain_a;
        qtypes_a[i]  = default_qtype_a;
    }

    // This is xmap boilerplate. Why do I have to write this?
    dns_num_ports_a = conf->source_port_last - conf->source_port_first + 1;
    setup_qtype_str_map_a();
    ...
    if (conf->probe_args &&
        ...
        if (strncasecmp(conf->probe_args, "raw", 3) == 0) {
            label_type_a = DNS_LTYPE_RAW;
            log_debug("dnsa", "raw label prefix");
        } else if (strncasecmp(conf->probe_args, "time", 4) == 0) {
            label_type_a = DNS_LTYPE_TIME;
            log_debug("dnsa", "time label prefix");
        } else if (strncasecmp(conf->probe_args, "random", 6) == 0) {
            label_type_a = DNS_LTYPE_RANDOM;
            log_debug("dnsa", "random label prefix");
        } else if (strncasecmp(conf->probe_args, "str", 3) == 0) {
            label_type_a     = DNS_LTYPE_STR;
            conf->probe_args = c;
            ...
            label_len_a = c - conf->probe_args;
            label_a     = xmalloc(label_len_a);
            strncpy(label_a, conf->probe_args, label_len_a);
            ++c;
            log_debug("dnsa", "label prefix: %s, len: %d", label_a,
                      label_len_a);
        } else if (strncasecmp(conf->probe_args, "dst-ip", 6) == 0) {
            label_type_a = DNS_LTYPE_SRCIP;
            log_debug("dnsa", "dst-ip label prefix");
        } else {
            log_error("dnsa", dnsa_usage_error);
            return EXIT_FAILURE;
        }
        ...
        if (strncasecmp(conf->probe_args, "text", 4) == 0) {
            if (load_question_from_str_a(c)) return EXIT_FAILURE;
        } else if (strncasecmp(conf->probe_args, "file", 4) == 0) {
            if (load_question_from_file_a(c)) return EXIT_FAILURE;
        } else {
            log_error("dnsa", dnsa_usage_error);
            return EXIT_FAILURE;
        }
        ...
        if (index_questions_a < num_questions_a) {
            log_error("dnsa", "more probes than questions configured. Add "
                              "additional probes.");
            return EXIT_FAILURE;
        }
    }

    if (label_type_a == DNS_LTYPE_RAW || label_type_a == DNS_LTYPE_STR)
        return build_global_dns_packets_a(domains_a, num_questions_a);
    else
        return EXIT_SUCCESS;
}

static int dnsa_global_cleanup(UNUSED struct state_conf *xconf,
                               UNUSED struct state_send *xsend,
                               UNUSED struct state_recv *xrecv) {
    if (dns_packets_a) {
        for (int i = 0; i < num_questions_a; i++) {
            if (dns_packets_a[i]) {
                free(dns_packets_a[i]);
            }
        }
        free(dns_packets_a);
    }
    ...
    if (dns_packet_lens_a) {
        free(dns_packet_lens_a);
    }
...
int dnsa_thread_init(void *buf, macaddr_t *src, macaddr_t *gw, void **arg_ptr) {
    memset(buf, 0, MAX_PACKET_SIZE);

    // Setup assuming num_questions_a == 0
    struct ether_header *eth_header = (struct ether_header *) buf;
    make_eth_header(eth_header, src, gw);

    struct ip *ip_header = (struct ip *) (&eth_header[1]);
    uint16_t   ip_len =
        sizeof(struct ip) + sizeof(struct udphdr) + dns_packet_lens_a[0];
    make_ip_header(ip_header, IPPROTO_UDP, ip_len);

    struct udphdr *udp_header = (struct udphdr *) (&ip_header[1]);
    uint16_t       udp_len    = sizeof(struct udphdr) + dns_packet_lens_a[0];
    make_udp_header(udp_header, udp_len);

    char *payload             = (char *) (&udp_header[1]);
    module_dnsa.packet_length = sizeof(struct ether_header) +
                                sizeof(struct ip) + sizeof(struct udphdr) +
                                dns_packet_lens_a[0];
    assert(module_dnsa.packet_length <= MAX_PACKET_SIZE);

    memcpy(payload, dns_packets_a[0], dns_packet_lens_a[0]);
...
int dnsa_make_packet(void *buf, size_t *buf_len, ipaddr_n_t *src_ip,
    ...
    port_h_t src_port = get_src_port(dns_num_ports_a, probe_num, validation);
    uint16_t dns_txid = get_dnsa_txid(validation, probe_num);

    if (label_type_a == DNS_LTYPE_RAW || label_type_a == DNS_LTYPE_STR) {
        // For num_questions_a == 1, we handle this in per-thread init. Do less
        // work
        if (num_questions_a > 1) {
            uint16_t ip_len = sizeof(struct ip) + sizeof(struct udphdr) +
                              dns_packet_lens_a[index];
            make_ip_header(ip_header, IPPROTO_UDP, ip_len);

            uint16_t udp_len = sizeof(struct udphdr) + dns_packet_lens_a[index];
            make_udp_header(udp_header, udp_len);

            char *payload = (char *) (&udp_header[1]);
            *buf_len      = sizeof(struct ether_header) + sizeof(struct ip) +
                       sizeof(struct udphdr) + dns_packet_lens_a[index];

            assert(*buf_len <= MAX_PACKET_SIZE);

            memcpy(payload, dns_packets_a[index], dns_packet_lens_a[index]);
        }
        ...
        case DNS_LTYPE_RANDOM: {
            ...
            dns_random_bytes_a(new_label, 8, charset_alpha_lower_a, 26, aes);
            new_label[8] = '\0';
            break;
        }
        ...
        default:
            log_fatal("dnsa", dnsa_usage_error);
            return EXIT_FAILURE;
        }
        ...
        qname_lens_a[index] = domain_to_qname_a(&qnames_a[index], new_domain);
        dns_packet_lens_a[index] = sizeof(dns_header) + qname_lens_a[index] +
                                   sizeof(dns_question_tail);
        if (dns_packet_lens_a[index] > DNS_SEND_LEN) {
            log_fatal("dnsa", "DNS packet bigger (%d) than our limit (%d)",
                      dns_packet_lens_a[index], DNS_SEND_LEN);
            return EXIT_FAILURE;
        }

        free(dns_packets_a[index]);

        dns_packets_a[index]            = xmalloc(dns_packet_lens_a[index]);
        dns_header        *dns_header_p = (dns_header *) dns_packets_a[index];
        char              *qname_p = dns_packets_a[index] + sizeof(dns_header);
        dns_question_tail *tail_p =
            (dns_question_tail *) (dns_packets_a[index] + sizeof(dns_header) +
                                   qname_lens_a[index]);
        ...
        // We have 1 question
        dns_header_p->qdcount = htons(1);
        memcpy(qname_p, qnames_a[index], qname_lens_a[index]);
        // Set the qtype to what we passed from args
        tail_p->qtype = htons(qtypes_a[index]);
        // Set the qclass to The Internet (TM) (R) (I hope you're happy
        // now Zakir)
        tail_p->qclass = htons(0x01);
        // MAGIC NUMBER. Let's be honest. This is only ever 1
...
void dnsa_print_packet(FILE *fp, void *packet) {
    ...
    uint16_t bytes_consumed = 0;
    char    *question_name  = get_name_a(data, data_len, (char *) dns_header_p,
    ...
    fprintf(fp,
            ...
            question_name, qtype_strs_a[qtype_qtype_to_strid_a[tail_p->qtype]]);
    fprintf(fp, "------------------------------------------------------\n");

    free(question_name);
}

int dnsa_validate_packet(const struct ip *ip_hdr, uint32_t len,
    ...
    if (ip_hdr->ip_p == IPPROTO_UDP) {
        ...
        if (!check_dns_src_port(sport, dns_num_ports_a, validation)) {
            return PACKET_INVALID;
        }
...
void dnsa_process_packet(const u_char *packet, uint32_t len, fieldset_t *fs,
    ...
    if (ip_header->ip_p == IPPROTO_UDP) {
        ...
        for (int i = 0; i < ntohs(dns_header_p->qdcount) && !err; i++) {
            err = process_response_question_a(
        ...
        // packet prevent this.
        log_fatal("dnsa", "Die. This can only happen if you "
...
probe_module_t module_dnsa = {
    ...
    .port_args       = 1,
    .global_init     = &dnsa_global_init,
    .thread_init     = &dnsa_thread_init,
    .make_packet     = &dnsa_make_packet,
    .print_packet    = &dnsa_print_packet,
    .validate_packet = &dnsa_validate_packet,
    .process_packet  = &dnsa_process_packet,
    .close           = &dnsa_global_cleanup,
