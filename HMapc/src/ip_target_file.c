...
int ip_target_set_thread_pos_tga(iterator_t *it) {
    int      i           = 0;
    uint64_t count       = 0;
    shard_t *sd          = get_shard(it, i);
    uint8_t  num_threads = get_num_threads(it);
    for (size_t line_idx = 0; line_idx < xconf.targets.size; line_idx++) {
        if (sd->ip_target_file_params.first == count) {
            sd->ip_target_file_params.pos = line_idx;
            i++;
            if (i >= num_threads) return EXIT_SUCCESS;
            sd = get_shard(it, i);
        }
...
static int recover_thread_file_params_tga(shard_t *shard) {
    if (shard->ip_target_file_params.port_current + 1 >=
        shard->ip_target_file_params.port_total)
        return EXIT_FAILURE;
    shard->ip_target_file_params.port_current++;
...
static int recover_thread_file_params_tga_(shard_t *shard) {
    if (shard->ip_target_file_params.index_current + 1 >=
        shard->ip_target_file_params.index_total) {
        shard->ip_target_file_params.index_current = 0;
        if (recover_thread_file_params_tga(shard)) return EXIT_FAILURE;
    } else {
        shard->ip_target_file_params.index_current++;
        shard->ip_target_file_params.current =
            shard->ip_target_file_params.first;
...
int ip_target_list_get_ip(void *ip, shard_t *shard) {
    if (shard->ip_target_file_params.current >=
        shard->ip_target_file_params.last)
        if (recover_thread_file_params_tga_(shard)) return EXIT_FAILURE;

    size_t idx = shard->ip_target_file_params.current;

    // Check if the current index is valid
    if (idx >= xconf.targets.size) {
        if (recover_thread_file_params_tga(shard)) return EXIT_FAILURE;
    }

    char  line[64];
    // Copy the IP from the IPVector object
    strcpy(line, xconf.targets.ips[idx]);
