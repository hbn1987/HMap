...
void enforce_range(const char *name, int v, int min, int max) {
    if (check_range(v, min, max) == EXIT_FAILURE) {
        log_fatal("hmap", "argument `%s' must be between %d and %d\n", name,
                  min, max);
    }
}
...
int data_download(char* data_type) {

    const char *hitlist = "hitlist";
    const char *aliasPrefix = "aliasPrefix";

    // Set the URL of the file to download
    const char *hitlist_url = 
        "https://alcatraz.net.in.tum.de/ipv6-hitlist-service/open/responsive-addresses.txt.xz";
    const char *aliasPrefix_url = 
        "https://alcatraz.net.in.tum.de/ipv6-hitlist-service/open/aliased-prefixes.txt.xz";
    ...
    if (!curl) {
        log_fatal("util", "failed to initialize libcurl");
    }

    if (strcmp(data_type, hitlist) == 0) {
        log_info("util", "downloading Gasser et al's IPv6 hitlist");
        sprintf(outfile, "Ghitlist_%s.xz", date_str);
        curl_easy_setopt(curl, CURLOPT_URL, hitlist_url);
    } else if (strcmp(data_type, aliasPrefix) == 0) { 
        log_info("util", "downloading Gasser et al's alias prefixes");
        sprintf(outfile, "GaliasPrefix_%s.xz", date_str);
        curl_easy_setopt(curl, CURLOPT_URL, aliasPrefix_url);
    } else {
        log_fatal("util", "the download can only specify parameters as "
                            "\"hitlist\" or \"aliasPrefix.\"");
    }
    ...
    if (!fp) {
        ...
        log_fatal("util", "failed to open file %s for writing", outfile);
    }
    ...
    if (res != CURLE_OK) {
        log_fatal("util", "failed to download: %s", curl_easy_strerror(res));
    }
    ...
    char cmd[256]; 
    snprintf(cmd, sizeof(cmd), "xz -d %s", outfile);

    // Execute the command
    int sysret = system(cmd);
    if (sysret == -1) {
        log_fatal("util", "file decompression failed");
    }
    return 1;
}
